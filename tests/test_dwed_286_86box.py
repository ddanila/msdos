    ...
    for mode in args.mode or ("low", "high"):
        ...
        with (case / "serial.log").open("w") as log:
            ...
            try:
                ...
                while b"DWED_286_FINISHED" not in (case / "serial.log").read_bytes():
        ...
        assert (
            run(["mtype", "-i", image, "::CPU.TXT"]).stdout.strip() == b"CPU MODEL PASS"
        )
        assert run(["mtype", "-i", image, "::HMA.TXT"]).stdout.strip() == b"HMA_PASS"
