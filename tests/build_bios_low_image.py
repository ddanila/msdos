...
def build(output, *, early=False, reservation_limit=0xfff0, tail_body=False, scan=False, rebase=False, compact=False, fail_tables=False, high_cds=False, fail_cds=False, cds_cache_case=None, cds_cache_negative=False, dispatch=False, characters=False, retire_characters=False, pack_headers=False, retire_media=False, paired_provider=None, pack_drive_graph=False, high_stack_pool=False, fail_stack_pool=False, retire_clock=False, retire_mux=False, compact_tracks=False, retire_swap=False, retire_ioctl_state=False):
    ...
    if early:
        ...
        seed = build(output, tail_body=tail_body, dispatch=dispatch, characters=characters,
                     ...
                     compact_tracks=compact_tracks, retire_swap=retire_swap, retire_ioctl_state=retire_ioctl_state)
        if rebase:
    ...
    if retire_ioctl_state:
        options += " -DBIOS_RETIRE_IOCTL_STATE=1"
    if scan:
    ...
    if retire_characters:
        ...
        for module in ("MSCON", "MSAUX", "MSLPT", "MSCLOCK"):
            ...
            objects.append(obj)
    if retire_ioctl_state:
        state = output / "BIOSIOST.OBJ"
        run([ROOT / "bin/jwasm-masm", options, f"BIOSIOST.ASM,{state};"], bios)
    ...
    if retire_ioctl_state:
        if not (symbols["BIOS_IOCTL_LOW_START"] == symbols["BIOS_IOCTL_LOW_END"]
                and symbols["END$"] <= symbols["BIOS_IOCTL_STATE_START"]
                < symbols["BIOS_IOCTL_STATE_END"] <= symbols["BIOS_SERVICE_START"]):
            raise ValueError("complete IOCTL fallback state must be outside retained low storage")
    if retire_swap:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--retire-ioctl-state", action="store_true", help="retain private IOCTL/format state only in the high disk owner")
