...
class_ptrs_hex=$(awk 'toupper($2) == "RESIDENT_CLASS_PTRS" { split($1, address, ":"); print address[2]; exit }' \
    "$ROOT/src/CMD/COMMAND/COMMAND.MAP")
critical_ptr_hex=$(awk 'toupper($2) == "RESIDENT_CRITICAL_PTR" { split($1, address, ":"); sub(/\*$/, "", address[2]); print address[2]; exit }' \
    "$ROOT/src/CMD/COMMAND/COMMAND.MAP")
...
hma_code_end_hex=$(awk 'toupper($2) == "HMA_CODE_END" { split($1, address, ":"); print address[2]; exit }' \
    "$ROOT/src/CMD/COMMAND/COMMAND.MAP")
[[ "$critical_ptr_hex" =~ ^[0-9A-Fa-f]{4}$ && "$catalog_start_hex" =~ ^[0-9A-Fa-f]{4}$ && "$dataresend_hex" =~ ^[0-9A-Fa-f]{4}$ \
...
nasm -DEXPECTED_CLASS_PTRS="0x$class_ptrs_hex" \
    -DEXPECTED_CRITICAL_PTR="0x$critical_ptr_hex" \
...
NO_XMS_QEMU_LOG="$OUT/hma-no-xms-qemu.log"
...
# Unix-domain socket paths are bounded independently of the artifact path.
NO_XMS_SOCKET_DIR=$(mktemp -d /tmp/msdos-hma-qmp.XXXXXX)
NO_XMS_QMP="$NO_XMS_SOCKET_DIR/qmp"
rm -f "$NO_XMS_SCREEN_LOG"
qemu-system-i386 \
    -display none -monitor none -machine pc -cpu 486 -m 16 \
    -drive if=floppy,index=0,format=raw,file="$NO_XMS_IMAGE",cache=writethrough \
    -boot a -serial file:"$NO_XMS_LOG" \
    -qmp unix:"$NO_XMS_QMP",server,nowait -no-reboot >"$NO_XMS_QEMU_LOG" 2>&1 &
no_xms_pid=$!
cleanup_no_xms() {
    kill "$no_xms_pid" 2>/dev/null || true
    wait "$no_xms_pid" 2>/dev/null || true
    rm -f "$NO_XMS_QMP"
    rmdir "$NO_XMS_SOCKET_DIR"
}
