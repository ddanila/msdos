...
nasm -DEXPECT_INSTALLED -f bin "$ROOT/tests/interlnk_offline_probe.asm" -o "$OFFLINE_PROBE"
...
printf 'LASTDRIVE=Z\r\nDEVICE=A:\\INTERLNK.EXE /DRIVES:2 /NOPRINTER /NOSCAN /BAUD:57600\r\n' | mcopy -o -i "$CLIENT_IMAGE" - ::CONFIG.SYS
...
python3 "$ROOT/tests/serial_fault_proxy.py" \
    --listen "$PROXY_PORT" --upstream "$PORT" --inject a5 --corrupt-sector 3 \
...
echo '  PASS: Interlnk rejects a corrupt payload, reconnects after header noise, and redirects two FAT volumes over COM2'
...
echo '  PASS: Interlnk remains installed offline and returns control without a server'
