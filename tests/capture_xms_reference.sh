...
SOURCE_IMAGE=$1
OUTPUT_LOG=$2
WORK_IMAGE=$(mktemp "${TMPDIR:-/tmp}/msdos-xms-reference.XXXXXX")
PROBE=$(mktemp "${TMPDIR:-/tmp}/msdos-xms-reference-probe.XXXXXX")
...
nasm -f bin "$ROOT/tests/xms_reference_probe.asm" -o "$PROBE"
export MTOOLS_NO_VFAT=1 MTOOLS_SKIP_CHECK=1
mcopy -o -i "$WORK_IMAGE" "$PROBE" ::XMSREF.COM
{
    printf '@ECHO OFF\r\n'
    printf 'CTTY AUX\r\n'
    printf 'XMSREF.COM\r\n'
...
if ! grep -q '^XMS_REFERENCE_END' "$OUTPUT_LOG"; then
    echo "XMS reference probe did not complete" >&2
    sed -n '1,180p' "$OUTPUT_LOG" >&2
    exit 1
fi
sed -n '/^XMS_REFERENCE_BEGIN/,/^XMS_REFERENCE_END/p' "$OUTPUT_LOG"
