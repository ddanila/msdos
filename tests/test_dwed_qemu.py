...
if (
    ...
    parser.error("database open probes require an editor built with --tests")
if (
    ...
    try:
        ...
        if name.startswith("table-row-"):
            ...
            if store_save:
                ...
                assert not diagnostic.strip(), diagnostic
                probe_log = run(["mtype", "-i", floppy, "::STRSAVE.LOG"]).stdout
                (d / "store-save.log").write_bytes(probe_log)
                assert b"PASS " in probe_log and b"FAIL" not in probe_log, probe_log
                assert (
                    run(["mtype", "-i", floppy, "::STRSAVE.OK"]).stdout.strip()
                    == b"STORE SAVE PASS"
                )
            ...
            if database_open:
                ...
                assert not diagnostic.strip(), diagnostic
                probe_log = run(["mtype", "-i", floppy, "::DBOPEN.LOG"]).stdout
                (d / "database-open.log").write_bytes(probe_log)
                assert b"PASS " in probe_log and b"FAIL" not in probe_log, probe_log
                assert (
                    run(["mtype", "-i", floppy, "::DBOPEN.OK"]).stdout.strip()
                    == b"DATABASE OPEN PASS"
                )
                row["database_open"] = probe_log.decode("ascii").strip()
            if database_probe:
