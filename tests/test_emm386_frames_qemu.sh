...
lowmem_sys="$OUT/int12-memory.sys"
nasm -f bin "$REPO_ROOT/tests/int12_memory_driver.asm" -o "$lowmem_sys"
...
# M10-M14 place the 64 KiB page frame below A000h and are valid only when
# conventional RAM ends at or below the requested frame.  Exercise them on
# the 512 KiB configuration for which DOS 6 documents these selectors.
for case_spec in 'M10 0x8000' 'M11 0x8400' 'M12 0x8800' \
    'M13 0x8c00' 'M14 0x9000'; do
    read -r option expected <<<"$case_spec"
    boot_img="$OUT/floppy-emm386-frame-${option}.img"
    probe_com="$OUT/emm386-frame-${option}.com"
    serial_log="$OUT/emm386-frame-${option}.log"
    cp "$FLOPPY" "$boot_img"
    nasm -f bin -DEXPECT_FRAME="$expected" \
        "$REPO_ROOT/tests/emm386_frame_probe.asm" -o "$probe_com"
    mcopy -o -i "$boot_img" "$probe_com" ::FRAME.COM
    mcopy -o -i "$boot_img" "$lowmem_sys" ::LOWMEM.SYS
    {
        printf 'DEVICE=A:\\LOWMEM.SYS\r\n'
        printf 'DEVICE=A:\\EMM386.EXE %s\r\n' "$option"
    } | mcopy -o -i "$boot_img" - ::CONFIG.SYS
    {
        printf '@ECHO OFF\r\n'
    ...
    timeout 35 qemu-system-i386 \
        ...
        >"$serial_log" 2>&1 || true
    if ! grep -q 'EMM386_FRAME_PASS' "$serial_log"; then
        echo "FAIL: EMM386 $option did not expose low page frame $expected" >&2
        sed -n '1,120p' "$serial_log"
        exit 1
    fi
done

echo "EMM386 M10-M14 low-memory page-frame selectors passed"
