...
.PHONY: test-command-step-qemu test-msd-qemu test-scandisk-qemu test-defrag-qemu test-defrag-fat16-qemu test-memmaker-qemu test-config-menu-qemu test-config-menu-input-qemu
...
$(FLOPPY): $(BOOT_BIN) $(IO_SYS) $(MSDOS_SYS) $(SYSMENU_OVL) $(COMMAND_COM) $(SYS_COM) $(FORMAT_COM) $(EXPAND_COM) $(SETUP_EXE) $(CHKDSK_COM) $(DEBUG_COM) $(MEM_EXE) $(FDISK_EXE) \
           ...
           $(MIRROR_COM) $(UNDELETE_COM) $(UNFORMAT_COM) $(CHOICE_COM) $(LOADFIX_COM) $(DELTREE_EXE) $(DEFRAG_EXE) $(MOVE_EXE) $(MSD_EXE) $(SCANDISK_EXE) $(MEMMAKER_EXE) \
