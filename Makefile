...
.PHONY: test-ru-profiles-qemu test-ru-install-qemu test-ru-keyboard-qemu test-ru-keyboard-records test-ru-country-records test-ru-country-qemu test-ru-font-sources test-ru-contract test-ru-cpi test-ru-display-qemu test-utility-parser-coverage-manifest test-keyboard-records test-country-records test-country-matrix-qemu test-internal-structure-coverage-manifest test-internal-structures-qemu
.PHONY: test-program-interface-coverage-manifest test-debug-command-coverage-manifest test-help-coverage-manifest test-expand test-choice test-loadfix-qemu test-exepack-qemu test-deltree-qemu test-move-qemu test-szdd-tool distribution test-distribution test-setup-qemu test-setup-floppy-qemu test-mirror-unformat-qemu test-undelete-qemu
...
test-xms-copy-windows-qemu:
	...
	python3 tests/test_xms_copy_windows_qemu.py --mapped --deny-later-page
	python3 tests/test_xms_copy_windows_qemu.py --mapped --deny-later-page --deny-destination
	python3 tests/test_xms_copy_windows_qemu.py --mapped --alias-overlap
...
.PHONY: test-xms-public-copy-qemu
test-xms-public-copy-qemu:
	python3 tests/test_xms_copy_windows_qemu.py --public-api --mode OFF
	python3 tests/test_xms_copy_windows_qemu.py --public-api --mode AUTO
...
test-bios-dispatch-hma-qemu:
	...
	python3 tests/test_bios_dispatch_hma_qemu.py --far-tables
...
test-ru-install-qemu: deploy
	python3 tests/test_ru_install_qemu.py

test-ru-keyboard-qemu: deploy $(SRC)/DEV/KEYBOARD/KEYBRD2.SYS
	python3 tests/test_ru_keyboard_qemu.py

test-ru-keyboard-records: $(SRC)/DEV/KEYBOARD/KEYBRD2.SYS
	python3 tests/test_ru_keyboard_records.py

test-ru-country-records: $(SRC)/DEV/COUNTRY/COUNTRY.SYS
	python3 tests/test_ru_country_records.py

test-ru-country-qemu: deploy $(SRC)/DEV/COUNTRY/COUNTRY.SYS $(SRC)/CMD/NLSFUNC/NLSFUNC.EXE
	python3 tests/test_ru_country_qemu.py

test-ru-contract:
	python3 tests/test_ru_contract.py

test-ru-cpi: $(SRC)/DEV/DISPLAY/EGA/EGA866.CPI
	python3 tests/test_ru_cpi.py

test-ru-display-qemu: deploy $(SRC)/DEV/DISPLAY/EGA/EGA866.CPI
	python3 tests/test_ru_display_qemu.py
...
.PHONY: test-baltic-country-resources-qemu
test: test-baltic-country-resources-qemu
...
test-baltic-installed-resources-qemu: deploy
	python3 tests/test_baltic_installed_resources_qemu.py

.PHONY: test-baltic-installed-workflows-qemu
test: test-baltic-installed-workflows-qemu
test-baltic-installed-workflows-qemu: deploy
	python3 tests/test_baltic_installed_workflows_qemu.py
...
test-baltic-files-qemu: deploy
	python3 tests/test_baltic_files_qemu.py
