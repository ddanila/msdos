...
.PHONY: test-command-step-qemu test-msd-qemu test-scandisk-qemu test-defrag-qemu test-defrag-fat16-qemu test-mem-dos6-qemu test-memmaker-qemu test-memmaker-rollback-qemu test-power-qemu test-config-menu-qemu test-config-menu-input-qemu
...
$(INC_DIR)/MSDATA.OBJ: $(INC_DIR)/MSDATA.ASM $(DOS_DIR)/MSINIT.ASM $(INC_DIR)/SETVER62.INC
...
test: $(KVIKDOS_SOFT_BIN) test-native-build-tools test-keyboard-records test-country-records test-szdd-tool test-distribution test-expand test-choice test-loadfix-qemu test-deltree-qemu test-move-qemu test-scandisk-qemu test-defrag-qemu test-defrag-fat16-qemu test-mem-dos6-qemu test-memmaker-qemu test-memmaker-rollback-qemu test-power-qemu test-setup-qemu test-himem-options-qemu test-himem-xms3-qemu test-batch-oracles test-oracle-mutation-coverage test-coverage-manifest test-int21-error-coverage-manifest test-runtime-coverage-manifest test-command-coverage-manifest test-utility-parser-coverage-manifest test-program-interface-coverage-manifest test-debug-command-coverage-manifest test-help-coverage-manifest test-dos-interrupt-coverage-manifest test-device-request-coverage-manifest
...
POWER_COM    := $(SRC)/CMD/POWER/POWER.COM
...
$(FLOPPY): $(BOOT_BIN) $(IO_SYS) $(MSDOS_SYS) $(SYSMENU_OVL) $(COMMAND_COM) $(SYS_COM) $(FORMAT_COM) $(EXPAND_COM) $(SETUP_EXE) $(CHKDSK_COM) $(DEBUG_COM) $(MEM_EXE) $(FDISK_EXE) \
           ...
           $(MIRROR_COM) $(UNDELETE_COM) $(UNFORMAT_COM) $(CHOICE_COM) $(LOADFIX_COM) $(DELTREE_EXE) $(DEFRAG_EXE) $(MOVE_EXE) $(MSD_EXE) $(SCANDISK_EXE) $(SCANDISK_INI) $(MEMMAKER_EXE) $(POWER_COM) $(POWER_EXE) \
