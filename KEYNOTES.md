...
## kvikdos: inplace_realloc Z-tail bug (fixed May 7 2026)
...
**39. triageError cross-group call in COMMAND — linker vs EXE2BIN tradeoff**
`INIT.ASM` (RESGROUP:INIT) still reaches `triageError` in TRANCODE via `DOSInvoke`, and `DOSInvoke` is hard-coded to emit a plain near `CALL`. That produces the familiar MS LINK `L2002` fixup overflow on the `INIT` call site because `triageError` lives in a different group. A direct `CALL FAR PTR triageError` does remove the linker error, but then `COMMAND.EXE` gains relocations and `EXE2BIN` refuses to convert it, so that is not a valid final fix for `COMMAND.COM`.

Current best direction: use the existing `triage_add` far-pointer thunk path already present in `INIT.ASM` / `UINIT.ASM` / `TMISC2.ASM` (`Triage_Init proc FAR`) instead of a direct far external call. That should preserve the relocation-free COM build while avoiding the cross-group near-call overflow.
