...
- **DEBUG.COM**: the old WASM blockers are now fixed upstream in the local Open Watcom fork:
  ...
  - Current hypothesis: there is still at least one more WASM MASM-compatibility bug in `DEBMES` code/data generation beyond symbol-surface differences. The next useful comparison target is the current fixed-WASM `DEBMES.OBJ` body against the MASM object, not just PUBDEF names.
