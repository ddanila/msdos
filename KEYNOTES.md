...
- **DEBUG.COM**: the old WASM blockers are now fixed upstream in the local Open Watcom fork:
  ...
  - Watcom-side investigation now points at structure-instantiation / field-offset handling for the `$M_RES_ADDRS <>` data block in `MSGSERV.ASM`, not symbol spelling or codegen in the callable stubs.
  - A closer synthetic assembler-side repro, shaped like the `$M_RES_ADDRS` tail (`DD -1`, class-pointer array, mixed `DW` / `DB` trailer), currently trips a WASM internal error in `asmins.c(2626)`. That is not yet the final minimal reproducer for the one-byte shift, but it strongly suggests the remaining `DEBMES` bug is in the same structure/data-emission area.
  - Current hypothesis: there is still at least one more WASM MASM-compatibility bug in `DEBMES` data layout beyond symbol-surface differences. The best next comparison target is no longer the whole object body, but specifically the structure/data-emission path that builds `$M_RT`.
