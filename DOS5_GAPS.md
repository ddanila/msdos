...
| `EMM386.EXE` | Partial DOS 5 EMM386 replacement | Driver-load `ON`/`OFF`/`AUTO`, `W=`, `Pn=`, `B=`, `L=`, `A=`, `H=`, and `D=` controls. The dual-purpose retail-named executable implements runtime status, `ON`, `OFF`, `AUTO`, `W=ON`, and `W=OFF`; driver loading implements pool size, `M1`-`M14`, `FRAME=`, `/P`, `I=`, `X=`, `RAM`, and `NOEMS`. |
