...
| `EMM386.EXE` | Partial DOS 5 EMM386 replacement | Driver-load `Pn=` and `B=` controls remain. Driver and runtime loading implement `ON`, `OFF`, `AUTO`, `W=ON`, and `W=OFF`; driver loading also implements pool size, `M1`-`M14`, `FRAME=`, `/P`, `I=`, `X=`, `L=`, `A=`, `H=`, `D=`, `RAM`, and `NOEMS`. The effect of `W=ON` on real Weitek hardware is unverified. |
