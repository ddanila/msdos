...
2. **E2E functional tests for read-only external tools** — MEM, SORT, FIND, TREE, FC, COMP can all be invoked directly under kvikdos (no disk writes). Good first batch to wire into `run_tests.sh` Section 6. **In progress:** kvikdos extended with --dos-version=4, INT 65h/12h/2Fh-B7h/EMS/EBDA/GETPID stubs, high-memory MMIO zeros. Full MS-DOS 4.0 build now works on both Linux and macOS (CL.EXE fixed via INT 21h/AH=87h GETPID stub). MEM.EXE prints correct output (exits 128, likely C runtime artifact). Still need: wire Section 6 tests, verify SORT/FIND/TREE/COMP work end-to-end.
