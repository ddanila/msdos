...
| `MSD` | Partial | A lean interactive/report implementation covers the reference-derived `/B`, `/I`, prompted `/F`, direct `/P`, and summary `/S` grammar plus processor generation, BIOS identity/date/equipment/base and extended memory and checksum, option-ROM inventory and checksum validation, ISA bus and DMA state, display/game-adapter state, active and alternate video adapters, text geometry, COM/LPT bases and BIOS status, UART generation and active divisor/rate, keyboard and mouse presence, DOS identity/OEM/serial/internal revision/location/boot drive/program path/environment strings, allocation/UMB/VERIFY/BREAK/code-page/country/date-format state, XMS/EMS versions, A20 state, available memory, resident MCB programs, Windows-installation discovery, IRQs, drivers, FAT-drive geometry, SUBST/JOIN/network mappings, video, and network reporting. The summary includes the retail field groups. Retail screen layout and exact field formatting or hardware-specific values remain. |
