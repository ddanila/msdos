...
| SMARTDrive | Partial | `SMARTDRV.EXE` controls live per-drive policy, delayed writes, flushing, sizing, and transfer-element selection over the compatibility driver. Runtime self-installation, effective read-ahead, CD-ROM caching, and a single dual-purpose binary remain. |
...
| `SMARTDRV` | Partial | `SMARTDRV.EXE` supplies status, fixed-drive read/read-write/off selection, `/C`, `/F`, `/L`, `/N`, `/Q`, `/R`, `/S`, `/U`, `/V`, `/X`, live cache resizing, cache locking, dirty-track write-behind, flushing, and an effective `/E` transfer-element setting over `SMARTDRV.SYS`. `/B` is validated and retained but does not yet change read-ahead. Remaining: runtime self-installation, effective read-ahead, CD-ROM caching, non-reboot shutdown paths, and one dual-purpose EXE. |
