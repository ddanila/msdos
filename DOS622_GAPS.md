...
| `MSCDEX` | Partial | `/D`, `/L`, `/M`, `/E`, `/S`, `/V`, and `/K` parsing; DOS device-header discovery; residency; drive assignment; and `INT 2Fh` functions 1500h, 1501h, 150Bh, 150Ch, and 150Dh are live. Remaining: request forwarding, ISO 9660 file access, directory and path services, audio/media-control IOCTLs, the rest of the multiplex API, effective EMS buffers, and network sharing. A hardware-specific CD-ROM device driver remains external. |
