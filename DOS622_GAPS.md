...
| `POWER` | Partial | The installable `POWER.EXE` driver and `POWER.COM` controller provide status, `OFF`, `STD`, `ADV`, and `ADV:MIN|REG|MAX`. The resident service recognizes INT 28h idleness and repeated empty keyboard polls, resets its detector on disk/video/DOS activity, performs level-dependent CPU halts, and makes APM idle calls when firmware is present. Remaining: exact APM lifecycle, thresholds, diagnostics, and reference API comparison. |
| `SCANDISK` | Partial | FAT12/FAT16 traversal, mirror and chain repair, lost-chain recovery, directory dot/parent/size/name/duplicate/attribute repair, `/AUTOFIX`, `/CHECKONLY`, named-file `/FRAGMENT` analysis, non-destructive free-space write/read `/SURFACE` verification, core prompting, repair logging, and byte-exact stale-safe `/UNDO` are live. Remaining: timestamp and remaining directory validation, occupied-cluster surface recovery, full `SCANDISK.INI`, exact multi-drive/parser/UI behavior, broader corruption/fault corpora, and compressed-volume integration in the DriveSpace epic. |
