...
| `POWER` | Partial | The installable `POWER.EXE` driver and controller provide status, `OFF`, `STD`, `ADV`, and `ADV:MIN|REG|MAX`. The resident service recognizes DOS and keyboard idleness, resets on hardware/DOS activity, performs level-dependent CPU halts, and uses APM when present. Its INT 2Fh 5400h/5401h/5403h/5481h identity and core results match a genuine 6.22 capture. Remaining: a single dual-purpose EXE, exact APM lifecycle, hardware thresholds, and retail diagnostics. |
...
| `SMARTDRV` | Partial | `SMARTDRV.EXE` supplies status, fixed-drive read/read-write/off selection, `/C`, `/F`, `/N`, `/Q`, `/R`, `/S`, `/V`, `/X`, real dirty-track write-behind, and explicit/timer/reboot flushing over `SMARTDRV.SYS`. `/B`, `/E`, `/L`, `/U`, and cache-size operands are parsed but do not yet change the resident layout. Remaining: runtime self-installation, effective sizing/read-ahead/element controls, CD-ROM caching, shutdown coverage, and one dual-purpose EXE. |
