...
| `SCANDISK` | Partial | FAT12/FAT16 traversal, allocation and directory repair, wildcard `/FRAGMENT`, free-space write/read `/SURFACE`, repair logging, stale-safe `/UNDO`, and the uncompressed-volume `SCANDISK.INI` environment/custom policies are live. Remaining: occupied-cluster surface recovery, `ScanTimeOut` plus graphical mouse/display behavior, exact multi-drive/parser/UI behavior, broader corruption/fault corpora, and `CheckHost`/compressed policies in the DriveSpace epic. |
