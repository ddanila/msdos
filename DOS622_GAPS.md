...
| Machine-to-machine and CD-ROM access | Partial | `INTERLNK.EXE` and `INTERSVR.EXE` provide byte-exact remote FAT access over COM1, while `MSCDEX.EXE` installs over an external device driver and exposes its core discovery API. Parallel transport, printer mapping, multi-drive discovery, CD-ROM filesystem access, and media control remain. |
...
| `INTERLNK` | Partial | The dual-purpose EXE installs as a block-device client, negotiates the remote BPB over COM1, mounts one server FAT volume, forwards sector reads/writes, and reports status. Remaining: `/DRIVES`, `/NOPRINTER`, `/COM` and `/LPT` selection, automatic port discovery, multiple drives, printer redirection, reconnect/unload, and transport fault recovery. |
| `INTERSVR` | Partial | A selected DOS volume is exported over `/COM:1`; geometry and byte-exact read/write requests work between independent machines. Remaining: multiple drives, printers, `/B`, `/V`, `/LPT`, `/X`, full connection UI/status, parallel transport, Alt+F4 shutdown, and `/RCOPY` bootstrap transfer. |
