...
| SMARTDrive | Partial | `SMARTDRV.EXE` controls live per-drive policy, delayed writes, flushing, sizing, transfer elements, and bounded read-ahead over the compatibility driver. Runtime self-installation, CD-ROM caching, and a single dual-purpose binary remain. |
...
| `MSCDEX` | Partial | `/D`, `/L`, `/M`, `/E`, `/S`, `/V`, and `/K` parsing; multi-subunit DOS device-header discovery; residency; drive assignment; and `INT 2Fh` functions 1500h through 1508h and 150Bh through 1510h are live. This includes per-subunit routing, ISO metadata, indexed VTOC reads, READ LONG conversion, an effective bounded `/M` sector cache retained conventionally or in EMS with `/E`, preservation of third-party EMS page maps, per-drive PVD/SVD selection with fallback, nested/version-aware ISO path traversal, and transparent driver requests. Media coverage includes play/stop/resume, eject/close, lock/unlock, reset, channel control, head position, volume size, media change, disc/track/Q-channel, audio status, and invalid-subunit errors. Remaining: DOS-visible redirector file access and network sharing. A hardware-specific CD-ROM device driver remains external. |
...
| `SMARTDRV` | Partial | `SMARTDRV.EXE` supplies status, fixed-drive read/read-write/off selection, `/C`, `/F`, `/L`, `/N`, `/Q`, `/R`, `/S`, `/U`, `/V`, `/X`, live cache resizing, cache locking, dirty-track write-behind, flushing, effective `/E` transfer elements, and `/B` read-ahead windows over `SMARTDRV.SYS`. Remaining: runtime self-installation, CD-ROM caching, non-reboot shutdown paths, and one dual-purpose EXE. |
