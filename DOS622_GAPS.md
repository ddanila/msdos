...
| `HIMEM.SYS` | XMS 2.00, the DOS 5 option set, DOS 6 `/EISA` memory discovery, `/V` and `/VERBOSE` diagnostics, and `/TESTMEM:ON|OFF` are covered. The default `/TESTMEM:ON` path writes and verifies complementary patterns across every byte of usable extended memory before publishing XMS; fault injection proves rejection and the OFF bypass. Exact 6.22 version/API identity, defaults, and remaining machine-option parity still require reference work. |
