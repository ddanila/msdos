...
| `DEFRAG` | Partial | `/U` relocates fragmented FAT12/FAT16 file chains into contiguous free runs; `/F` compacts movable clusters and `/S[:]order` physically sorts directory entries while preserving byte-exact files. Nested and hidden-file policy, `/B` reboot, `/SKIPHIGH`, and distinct `/LCD`, `/BW`, and `/G0` text presentations are covered. Divergent FAT copies, lost allocations, invalid chains, full volumes, and invalid boot records are refused without writes; success and errorlevels 2, 3, 4, and 7 have image-level contracts. Every `/U` and `/F` relocation write boundary is fault-injected and proven byte-preserving and ScanDisk-recoverable. Remaining: physical read/write errorlevels 5 and 6, directory-limit errorlevel 9, and the reference full-screen interface. |
