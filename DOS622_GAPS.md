...
| `MSD` | Partial | A lean interactive/report implementation covers the reference-derived `/B`, `/I`, prompted `/F`, direct `/P`, and summary `/S` grammar plus BIOS identity/date/equipment/base memory/display/game-adapter state, COM/LPT bases, keyboard and mouse presence, DOS identity/OEM, allocation/UMB/VERIFY/BREAK/code-page/country/date-format/environment state, XMS/EMS versions, A20 state, and available memory, IRQ, driver, FAT-drive geometry, SUBST/JOIN/network mappings, video, and network reporting. Retail screen layout, exhaustive device detail, and field-by-field report parity remain. |
