...
| `DEFRAG` | Partial | `/U` relocates fragmented FAT12/FAT16 file chains into contiguous free runs; `/F` compacts movable clusters and `/S[:]order` physically sorts directory entries while preserving byte-exact files. Nested and hidden-file policy, `/B` reboot, `/SKIPHIGH`, and distinct `/LCD`, `/BW`, and `/G0` text presentations are covered. Divergent FAT copies, lost allocations, invalid chains, full volumes, invalid boot records, and oversized sortable directories are refused without writes; success and errorlevels 2, 3, 4, 7, and 9 have image-level contracts. Every `/U` and `/F` relocation write boundary is fault-injected and proven byte-preserving and ScanDisk-recoverable. Remaining: physical read/write errorlevels 5 and 6 and the reference full-screen interface. |
...
| `MSD` | Partial | A lean interactive/report implementation covers the reference-derived `/B`, `/I`, prompted `/F`, direct `/P`, and summary `/S` grammar plus processor generation, BIOS identity/date/equipment/base memory and checksum, option-ROM inventory and checksum validation, ISA bus and DMA state, display/game-adapter state, active and alternate video adapters, text geometry, COM/LPT bases and BIOS status, UART generation and active divisor/rate, keyboard and mouse presence, DOS identity/OEM, allocation/UMB/VERIFY/BREAK/code-page/country/date-format/environment state, XMS/EMS versions, A20 state, available memory, resident MCB programs, Windows-installation discovery, IRQs, drivers, FAT-drive geometry, SUBST/JOIN/network mappings, video, and network reporting. Retail screen layout and field-by-field report parity remain. |
