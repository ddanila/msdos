...
| `MSCDEX` | Partial | `/D`, `/L`, `/M`, `/E`, `/S`, `/V`, and `/K` parsing; DOS device-header discovery; residency; drive assignment; `INT 2Fh` functions 1500h through 1508h and 150Bh through 150Eh; and 1510h are live. This includes ISO primary-descriptor metadata names, indexed VTOC reads and classification, production no-op debug calls, absolute READ LONG conversion, per-drive PVD/shift-JIS SVD preference, and direct strategy/interrupt request forwarding. Remaining: DOS-visible ISO 9660 file access, directory/path service 150Fh, audio/media-control IOCTLs, effective EMS buffers, and network sharing. A hardware-specific CD-ROM device driver remains external. |
