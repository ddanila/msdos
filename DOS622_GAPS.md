...
| `SCANDISK` | Partial | FAT12/FAT16 traversal, mirror and chain repair, lost-chain recovery, directory dot/parent/size repair, `/AUTOFIX`, `/CHECKONLY`, named-file `/FRAGMENT` analysis, non-destructive free-space write/read `/SURFACE` verification, core prompting, repair logging, and byte-exact stale-safe `/UNDO` are live. Remaining: duplicate/name/timestamp directory validation, occupied-cluster surface recovery, full `SCANDISK.INI`, exact multi-drive/parser/UI behavior, broader corruption/fault corpora, and compressed-volume integration in the DriveSpace epic. |
