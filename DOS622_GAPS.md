...
| Memory optimization | Present | Reversible MemMaker startup-file/reboot workflow with measured baseline, post-CONFIG, and post-AUTOEXEC memory states, staged progress/results, and per-candidate Custom driver/TSR placement. MemMaker distinguishes transient EXEC demand from resident size and pins bounded programs to safe UMB regions; unbounded COM/MZ programs deliberately retain ordinary `LH` placement. Remaining DOS 6 EMM386/MEM/HIMEM differentials are tracked separately. |
...
| Diagnostics and power | Partial | `MSD` ships with mouse/keyboard interactive categories and complete report modes. POWER has an installable driver, all controller modes, application-idle and keyboard-poll detection, disk/video/DOS activity resets, CPU-idle action, APM discovery, and reference-derived status and API diagnostics. Physical POWER calibration remains. |
