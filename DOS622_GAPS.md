...
| Machine-to-machine and CD-ROM access | Partial | `INTERLNK.EXE` and `INTERSVR.EXE` discover and redirect multiple remote FAT volumes with byte-exact access over COM1, while `MSCDEX.EXE` installs over an external device driver and exposes its core discovery API. Parallel transport, printer mapping, CD-ROM filesystem access, and media control remain. |
...
| `INTERLNK` | Partial | The dual-purpose EXE installs as a multi-unit block-device client, discovers up to five server volumes, negotiates an independent BPB for each, forwards sector reads/writes, and reports the live drive count over COM1. CONFIG.SYS accepts an effective `/DRIVES:1..5` cap plus drive-only `/NOPRINTER` and `/COM:1`; bounded UART waits let startup and I/O fail instead of hanging without a peer. Remaining: other `/COM` and `/LPT` selection, automatic port discovery, printer redirection, automatic reconnect/resynchronization, and unload. |
| `INTERSVR` | Partial | Up to five selected DOS volumes are exported over `/COM:1`; discovery, per-volume geometry, and byte-exact read/write requests work between independent machines. Floppy exports use physical BIOS I/O so A: and B: remain distinct even under single-drive DOS aliasing, and UART waits are bounded. Remaining: printers, `/B`, `/V`, `/LPT`, `/X`, full connection UI/status, parallel transport, Alt+F4 shutdown, protocol resynchronization, and `/RCOPY` bootstrap transfer. |
| `LOADFIX` | Present | Placement above the first 64 KiB, argument forwarding, and child exit propagation are covered. |
| `MEMMAKER` | Partial | Unattended Express rewriting, byte-exact backups and `/UNDO`, automatically scheduled reboot `/SESSION`, `/SWAP`, `/W` status, memory-manager insertion, DEVICEHIGH/INSTALLHIGH, and eligible TSR `LH` conversion are live. Fault injection covers every optimization commit boundary. Remaining: measured multi-pass placement and size ordering, interactive Custom choices, Windows SYSTEM.INI handling, and exact retail UI/status behavior. |
...
| `MSCDEX` | Partial | `/D`, `/L`, `/M`, `/E`, `/S`, `/V`, and `/K` parsing; DOS device-header discovery; residency; drive assignment; `INT 2Fh` functions 1500h through 1508h and 150Bh through 1510h are live. This includes ISO primary-descriptor metadata, indexed VTOC reads, READ LONG conversion, per-drive descriptor preference state, nested and version-aware ISO path traversal with raw directory-record return, and direct strategy/interrupt request forwarding. Remaining: DOS-visible redirector file access, effective SVD selection, audio/media-control IOCTLs, effective EMS buffers, and network sharing. A hardware-specific CD-ROM device driver remains external. |
...
| `SMARTDRV` | Partial | `SMARTDRV.EXE` supplies status, fixed-drive read/read-write/off selection, `/C`, `/F`, `/N`, `/Q`, `/R`, `/S`, `/V`, `/X`, live cache resizing within its resident allocation, real dirty-track write-behind, and explicit/timer/reboot flushing over `SMARTDRV.SYS`. `/B`, `/E`, `/L`, and `/U` are parsed but do not yet change cache behavior. Remaining: runtime self-installation, effective read-ahead/element controls, CD-ROM caching, shutdown coverage, and one dual-purpose EXE. |
...
| `EMM386` | DOS 6 command-mode statistics and diagnostics, ON/OFF/AUTO state transitions, application-handle-driven AUTO activation/release, W= handling, EMS/UMB modes, memory regions, frame selection, and LIM 4.0 APIs are covered. Remaining compatibility limits are the low-memory-only M10-M14 frame placements, real Weitek behavior, and physical-hardware timing validation. |
