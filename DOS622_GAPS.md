...
| Memory optimization | Partial | Strong HMA/UMB base plus a reversible MemMaker startup-file/reboot workflow with real post-CONFIG UMB/conventional measurement; per-candidate multi-pass placement and remaining DOS 6 EMM386/MEM/HIMEM differentials remain. |
...
| `INTERLNK` | Partial | The dual-purpose EXE installs as a multi-unit block-device client, discovers up to five server volumes, negotiates an independent BPB for each, forwards sector reads/writes, and reports the live drive count and port. CONFIG.SYS accepts an effective `/DRIVES:1..5` cap, drive-only `/NOPRINTER`, and `/COM:1..4`; bounded UART waits prevent hangs without a peer. Remaining: `/LPT` and automatic port discovery, printer redirection, automatic reconnect/resynchronization, and unload. |
...
| `MEMMAKER` | Partial | Unattended Express rewriting, byte-exact backups and `/UNDO`, automatically scheduled reboot `/SESSION`, `/SWAP`, memory-manager insertion, DEVICEHIGH/INSTALLHIGH, and eligible TSR `LH` conversion are live. The reboot pass measures the actual largest UMB and conventional blocks through DOS allocation APIs, applies `/W` reserve accounting, and restores allocation state; fault injection covers every commit boundary. Remaining: per-candidate multi-pass placement and size ordering, interactive Custom choices, Windows SYSTEM.INI handling, and exact retail UI behavior. |
...
| `MSCDEX` | Partial | `/D`, `/L`, `/M`, `/E`, `/S`, `/V`, and `/K` parsing; multi-subunit DOS device-header discovery; residency; drive assignment; and `INT 2Fh` functions 1500h through 1508h and 150Bh through 1510h are live. This includes per-subunit routing, ISO metadata, indexed VTOC reads, READ LONG conversion, effective per-drive PVD/SVD selection with PVD fallback, nested/version-aware ISO path traversal, and direct driver requests. Remaining: DOS-visible redirector file access, audio/media-control IOCTLs, effective EMS buffers, and network sharing. A hardware-specific CD-ROM device driver remains external. |
