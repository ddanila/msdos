...
| `MSCDEX` | Partial | `/D`, `/L`, `/M`, `/E`, `/S`, `/V`, and `/K` parsing; DOS device-header discovery; residency; drive assignment; `INT 2Fh` functions 1500h, 1501h, 1508h, 150Bh, 150Ch, and 150Dh; and 1510h request forwarding through driver strategy/interrupt entry points are live. Function 1508h converts absolute reads into validated READ LONG driver packets and returns DOS drive/not-ready errors. Remaining: ISO 9660 file access, directory and path services, audio/media-control IOCTLs, the rest of the multiplex API, effective EMS buffers, and network sharing. A hardware-specific CD-ROM device driver remains external. |
