...
| SMARTDrive | Partial | `SMARTDRV.EXE` self-installs its embedded cache at runtime and controls live per-drive policy, delayed writes, flushing, sizing, transfer elements, and bounded read-ahead. CD-ROM caching and CONFIG.SYS use of the same EXE remain. |
...
| `SMARTDRV` | Partial | `SMARTDRV.EXE` self-installs its embedded driver when needed and supplies status, fixed-drive read/read-write/off selection, `/C`, `/F`, `/L`, `/N`, `/Q`, `/R`, `/S`, `/U`, `/V`, `/X`, live resizing, cache locking, dirty-track write-behind, flushing, `/E` transfer elements, and `/B` read-ahead. `SMARTDRV.SYS` remains the CONFIG.SYS form. Remaining: CD-ROM caching, non-reboot shutdown paths, and CONFIG.SYS loading through the same EXE. |
