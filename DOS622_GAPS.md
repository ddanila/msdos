...
| `SCANDISK` | Partial | FAT12/FAT16 traversal, mirror and chain repair, lost-chain recovery, directory dot/parent/size/name/duplicate/attribute/timestamp/starting-cluster repair, misplaced-label removal, `/AUTOFIX`, `/CHECKONLY`, named-file `/FRAGMENT`, free-space write/read `/SURFACE`, repair logging, and stale-safe `/UNDO` are live. Remaining: occupied-cluster surface recovery, full `SCANDISK.INI`, exact multi-drive/parser/UI behavior, broader corruption/fault corpora, and compressed-volume integration in the DriveSpace epic. |
