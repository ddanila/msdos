...
| Machine-to-machine and CD-ROM access | Partial | `MSCDEX.EXE` installs over an external device driver and exposes its core discovery API. `INTERLNK.EXE`, `INTERSVR.EXE`, CD-ROM filesystem access, and media control remain missing. |
...
| `MEMMAKER` | Partial | Unattended Express rewriting, byte-exact backups and `/UNDO`, reboot plus `/SESSION`, `/SWAP`, `/W` status, memory-manager insertion, DEVICEHIGH/INSTALLHIGH, and eligible TSR `LH` conversion are live. Fault injection covers every optimization commit boundary. Remaining: measured multi-pass placement and size ordering, interactive Custom choices, automatic session scheduling, Windows SYSTEM.INI handling, and exact retail UI/status behavior. |
