...
| `INTERLNK` | Partial | The dual-purpose EXE installs as a multi-unit block-device client, discovers up to five server volumes, negotiates an independent BPB for each, forwards sector reads/writes, and reports the live drive count and port. CONFIG.SYS accepts an effective `/DRIVES:1..5` cap, drive-only `/NOPRINTER`, and `/COM:1..4`; bounded UART waits prevent hangs without a peer. Failed I/O marks the link offline, later requests reconnect lazily, and `INTERLNK /RECONNECT` explicitly repeats discovery and BPB negotiation. Remaining: `/LPT` and automatic port discovery, printer redirection, deeper mid-packet fault recovery, and unload. |
...
| `MSCDEX` | Partial | `/D`, `/L`, `/M`, `/E`, `/S`, `/V`, and `/K` parsing; multi-subunit DOS device-header discovery; residency; drive assignment; and `INT 2Fh` functions 1500h through 1508h and 150Bh through 1510h are live. This includes per-subunit routing, ISO metadata, indexed VTOC reads, READ LONG conversion, an effective bounded `/M` resident sector cache, per-drive PVD/SVD selection with PVD fallback, nested/version-aware ISO path traversal, and direct driver requests. Remaining: DOS-visible redirector file access, audio/media-control IOCTLs, EMS-backed `/E` buffers, and network sharing. A hardware-specific CD-ROM device driver remains external. |
