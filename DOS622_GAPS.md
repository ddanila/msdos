...
| `MSD` | Partial | A lean interactive/report implementation covers the reference-derived `/B`, `/I`, prompted `/F`, direct `/P`, and summary `/S` grammar plus BIOS identity/date/equipment/base memory/display/game-adapter state, COM/LPT bases, keyboard and mouse presence, DOS allocation/UMB/VERIFY/BREAK/code-page/environment state, XMS/EMS versions and available memory, IRQ, driver, FAT-drive geometry, SUBST/JOIN/network mappings, video, network, and OS reporting. Retail screen layout, exhaustive device detail, and field-by-field report parity remain. |
