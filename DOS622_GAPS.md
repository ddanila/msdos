...
| SMARTDrive | Partial | The dual-purpose `SMARTDRV.EXE` loads through CONFIG.SYS or self-installs at runtime, then controls live per-drive policy, delayed writes, flushing, sizing, transfer elements, and bounded read-ahead. CD-ROM caching remains. |
...
| `SMARTDRV` | Partial | The dual-purpose `SMARTDRV.EXE` loads through CONFIG.SYS or self-installs its embedded driver, then supplies status, fixed-drive read/read-write/off selection, `/C`, `/F`, `/L`, `/N`, `/Q`, `/R`, `/S`, `/U`, `/V`, `/X`, live resizing, locking, write-behind, flushing, `/E` transfer elements, and `/B` read-ahead. Remaining: CD-ROM caching and non-reboot shutdown paths. |
