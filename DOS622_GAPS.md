...
| `INTERLNK` | Partial | The dual-purpose EXE installs as a multi-unit block-device client, discovers up to five server volumes, negotiates an independent BPB for each, forwards sector reads/writes, and reports the live drive count over COM1. Remaining: `/DRIVES`, `/NOPRINTER`, `/COM` and `/LPT` selection, automatic port discovery, printer redirection, reconnect/unload, and transport fault recovery. |
