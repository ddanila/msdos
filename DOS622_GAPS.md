...
| SMARTDrive | Partial | The dual-purpose `SMARTDRV.EXE` loads through CONFIG.SYS or self-installs at runtime, then controls live per-drive policy, delayed writes, flushing, sizing, transfer elements, bounded read-ahead, and MSCDEX CD caching with `/U` opt-out. Non-reboot shutdown notification remains. |
...
| `SMARTDRV` | Partial | The dual-purpose `SMARTDRV.EXE` loads through CONFIG.SYS or self-installs its embedded driver, then supplies status, fixed-drive read/read-write/off selection, `/C`, `/F`, `/L`, `/N`, `/Q`, `/R`, `/S`, `/U`, `/V`, `/X`, live resizing, write-behind, flushing, `/E` transfer elements, and `/B` read-ahead. `/L` keeps runtime placement low and `/U` suppresses MSCDEX CD caching. Remaining: non-reboot shutdown notification. |
