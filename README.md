...
2. **Test**: Validate build outputs with integration tests — file existence checks, SHA256 golden checksums, COMMAND.COM smoke tests, /? help smoke tests for all 38 CMD tools, and E2E functional tests (MEM, FIND, FC, TREE) under kvikdos.
...
5. **E2E test**: Boot the floppy in QEMU and test built-in commands (VER, ECHO, SET, PATH, DIR, VOL via `make test-builtins`), plus `FORMAT B:` → `SYS B:` on a blank image to verify it boots independently (`make test-sys`).
6. **CI**: Automated `make test` + `make deploy` + `make verify` + `make test-sys` + `make test-builtins` in GitHub Actions on every push.
...
- `tests/` — integration tests (file existence, SHA256 golden checksums, /? help, kvikdos E2E, QEMU built-in E2E, SYS e2e)
