...
def run_case(work,base,profile,args,core,locale=None,installed=None):
    ...
    else:shutil.copyfile(installed['image'] if installed else base,image)
    ...
    for name in ('IO.SYS','MSDOS.SYS','COMMAND.COM'):
        assert hashlib.sha256(read(name)).hexdigest()==core[name],name
    for source,name in [('CMD/EDLIN/EDLIN.COM','EDLIN.COM'),('CMD/FIND/FIND.EXE','FIND.EXE'),
        ...
        if installed:assert read('DOS/'+name)==data,name
        else:put(name,data)
    ...
    if installed:
        assert read('CONFIG.SYS')==installed['config']
    else:
    ...
    if installed:
        actions=['C:','CD \\', 'PATH C:\\DOS', 'PROFILE.COM > AUX', *installed['recipe']]
    batch=['@ECHO OFF']
    ...
    for name in ['SHELL.TXT','EDIT.TXT','COPY.TXT','PIPE.TXT']:
        value=read(name);(case/name).write_bytes(value);files[name]=value.hex()
    expected=(sample+'\r\n'+second+'\r\n').encode(encoding)
