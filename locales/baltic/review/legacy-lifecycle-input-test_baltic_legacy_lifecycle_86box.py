...
def legacy_cases(cases):
    ...
    for original in cases:
        ...
        if 'ack_after' in step:
            # Legacy BIOS toggles Caps with Caps Lock; unlike enhanced German
            # handling, a Shift stroke does not clear the BIOS Caps flag.
            if keys[-2:] == tap('shift'):
                keys[-2:] = tap('caps_lock')
            # Keep the physical Caps-off stroke after the barrier.
