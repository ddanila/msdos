...
class VNCKeyboard:
    ...
    KEYS = {'alt':0xffe9,'shift':0xffe1,'shift_r':0xffe2,'ctrl':0xffe3,
            ...
            'backspace':0xff08,'tab':0xff09,'spc':32,'f1':0xffbe,'f2':0xffbf,'f3':0xffc0,
...
def run_case(work, args, core, kind, locale=None):
    ...
    if args.machine=='xt83':
        ...
        image.unlink()
        # Compact Baltic matrices hold many small malformed-library fixtures.
        # Single-sector clusters retain 360 KiB XT geometry with less slack.
        clusters=['-c','1'] if locale and locale.get('compact_batch') else []
        subprocess.run(['mformat','-C','-f','360',*clusters,'-B',str(boot),'-i',str(image),'::'],check=True)
    ...
    prepared = [page] + (locale.get('extra_pages', []) if locale else [])
    assert len(prepared) == len(set(prepared))
    for extra in prepared[1:]:
        copy(f'EGA{extra}.CPI', ROOT/f'src/DEV/DISPLAY/EGA/EGA{extra}.CPI')
    put('CONFIG.SYS',f'COUNTRY={country:03d},{page},COUNTRY.SYS\r\nDOS=LOW\r\nDEVICE=DISPLAY.SYS CON=(EGA,437,({len(prepared)},3))\r\nNUMLOCK=ON\r\n'.encode('ascii'))
    actions=[country_probe,'NLSFUNC']
    actions += [f'MODE CON CP PREPARE=(({","*slot}{value}) EGA{value}.CPI)'
                for slot,value in enumerate(prepared)]
    actions += [f'MODE CON CP SELECT={page}', 'KEYB '+selection,
                'TYPECHK.COM','CPCHK.COM',country_probe,'PROBE.COM']
    actions=[(action,False) for action in actions]
    probe_passes=type_passes=1
    if args.suite=='lifecycle':
        prepare=locale['lifecycle'] if locale else prepare_lifecycle
        cases,extra,probe_passes=prepare(case,kind,put)
        actions=actions[:-1]+extra
        type_passes=sum(action=='TYPECHK.COM' for action,rejected in actions)
    if args.machine=='xt83' and locale and locale.get('compact_batch'):
        # These private tests have no CONFIG menu and lifecycle replaces the
        # initial PROBE.COM with its own probes. Keep all mutant libraries.
        unused=['SYSMENU.OVL']+(['PROBE.COM'] if args.suite=='lifecycle' else [])
        subprocess.run(['mdel','-i',str(image),*['::'+name for name in unused]],check=True)
    batch=['@ECHO OFF','CTTY AUX']
    for action,rejected in actions:
        if not (locale and locale.get('compact_batch')):
            batch.append('ECHO RUN '+action)
        batch += [action,
        ...
        try:
            ...
            for index,step in enumerate(cases):
                ...
                while marker not in log.read_bytes():
                    ...
                    if index==0 and time.monotonic()>=reconnect_at and b'RUN ' not in log.read_bytes() and b'RU_COUNTRY_PASS' not in log.read_bytes():
                        ...
                        reconnect_at=time.monotonic()+10
                    time.sleep(.02)
                barrier=step.get('ack_after')
                if barrier is None:
                    vnc.send(step['keys'])
                else:
                    vnc.send(step['keys'][:barrier])
                    armed=f'RU_KEY_ARMED {index:04X}\r\n'.encode()
                    deadline=time.monotonic()+15
                    while armed not in log.read_bytes():
                        if proc.poll() is not None or time.monotonic()>deadline:
                            raise AssertionError(f'pending-key barrier failed: {step}: {log}')
                        time.sleep(.01)
                    vnc.send(step['keys'][barrier:])
