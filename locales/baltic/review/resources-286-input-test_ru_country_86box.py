...
def run_case(work,args,core,name,country_config,actions,probe_files=None,
             font_page=866,country_source=None,expected_failure=None,expected_markers=None,timeout=240):
    case=work/name
    case.mkdir()
    image=case/'test.img'
    ...
    with log.open('wb') as output:
        result=subprocess.run([str(args.emulator.resolve()),'-N','-O',str(case/'global.cfg'),'-P',str(case),
                              ...
                              stdin=subprocess.DEVNULL,stdout=output,stderr=subprocess.STDOUT,timeout=timeout)
    data=log.read_bytes()
    counts={}
    if failure_stage:
