...
runtime-sized sparse `Pn=` exceptions, and a runtime-sized DMA page list leave
607,168 bytes, an 11,568-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 96 |
...
| **Total** | **VC largest-block gap** | **11,568** |
...
usable UMB space. The current result is 607,168 bytes, so 11,568 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 96 bytes | 607,264 bytes |
...
Matching only the three measured component sizes recovers at most 2,464 bytes
...
Treat the 11,568-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 11,568 - EMM386 gain - HIMEM gain - COMMAND gain
...
2,464 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 3,488 bytes, still leaving **8,080 bytes**. The final route must
...
| 1 | Complete the small EMM386 low gateway backed by locked XMS | Local EMM386 now costs 96 bytes more than retail; services, return and A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity handling, and the protected `GoVirtual` continuation execute from locked XMS; command-mode client and debugger-only descriptors are nonresident | hundreds of bytes locally; better-than-retail remains possible | Remaining transition, DMA, and fault gateways; inactive `AUTO`; all EMS maps |
| 2 | Compact EMM386 runtime-sized metadata and alignment | Physical-page IDs and the DMA page list now scale with selected options; measured subranges can supplement the gateway redesign | tens to hundreds of bytes per item | Full `H=`/`A=`/`D=` ranges and EMS 4.0 formats |
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 9,664 bytes more than retail; after measured manager and table differences, 8,096 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `9,664 + 880 + 0 + 1,024 = 11,568`; the component census isolates 8,096 bytes for A3 |
...
boundary and 17,372 non-low bytes follow it. The report now accounts for
the complete 1,831-byte static low-image address range: a 426-byte real-mode
gateway, 224-byte GDT, 504-byte `_DATA`, 177-byte constants, 10-byte BSS,
...
For the fixed configuration it then accounts for the complete 1,874-byte VDATA:
512 bytes of saved maps, 256 bytes of handle records, 512 bytes of names,
512 bytes of page arrays, a two-byte default DMA page list, and 80 bytes of
normal/alternate register sets. An
option-dependent alignment gap precedes the 512-byte protected stack; the fixed
configuration uses 7 alignment bytes, bringing the computed installed
allocation to 4,224 bytes, exactly matching `MEM /D`.
Command-line counts
...
1. **Finish attribution.** Extend the existing owner-level reconciliation with
   ...
   accounted; the 8,096-byte DOS/BIOS remainder still needs safe subranges.
...
The current milestone reproduces 607,168 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 28,624 bytes versus retail's
18,880, a 9,744-byte difference that includes the memory managers and retained
...
| 1 | Complete the EMM386 low gateway | 96-byte excess over retail; services, return and A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity, and the `GoVirtual` continuation execute from locked XMS; command-mode client code and debugger descriptors are nonresident | Redesign the remaining transition, DMA, and fault gateways; preserve inactive `AUTO` and every EMS map |
| 1 | Compact EMM386 metadata while changing that boundary | The loader stack is discarded, duplicate PTE offsets and inverse segment index are gone, physical-window segments use byte 16 KiB indexes, public `Pn=` identifiers and the DMA page list are runtime-sized, protected entry/services/helpers are high, the two-function control gateway is table-free, the production GDT omits six debugger descriptors, `NOHIMEM` hooks are absent, and fixed selector/parity operations are compact; remaining work is included in the 96-byte excess | Take independently testable descriptor, VDATA, table, and alignment wins without delaying the gateway design |
...
| 1 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is now 96 bytes above retail after moving services, return/A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity handling, and the `GoVirtual` continuation high, making command-mode client code and debugger descriptors nonresident, and sizing physical-window IDs and DMA pages at runtime | Continue toward the smaller owner by redesigning remaining transition, fault, and DMA entries; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 now occupies 4,224 bytes here versus 4,128 in retail. Potential further
...
| EMM386 | 4,224 | 4,128 | 96 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 12,608 | 11,040 | 1,568 |

`MEM` reports 12,720 and 11,168 bytes after each block's arena overhead. Both
...
The current 607,168-byte largest block is 11,568 bytes (1.9%) below retail and
