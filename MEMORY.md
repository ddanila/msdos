...
warm-reboot vector restoration leave 607,744 bytes, a 10,992-byte gap.
...
is another oversized component. The 7,616-byte DOS/BIOS row is an exact
...
usable UMB space. The current result is 607,744 bytes, so 10,992 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 0 bytes | 607,744 bytes |
| HIMEM resident allocation | 1,488 bytes | 609,232 bytes |
| COMMAND resident allocation | 880 bytes | 610,112 bytes |
| Layout and conventional ceiling | 8,624 bytes | 618,736 bytes |
...
Treat the 10,992-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 10,992 - EMM386 gain - HIMEM gain - COMMAND gain
...
raises that to 3,392 bytes, still leaving **7,600 bytes**. The final route must
therefore include at least one of these outcomes:

- recover at least 7,600 bytes from retained DOS/BIOS layout and fragmentation;
- make EMM386, HIMEM, or COMMAND collectively at least 7,600 bytes smaller than
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 9,088 bytes more than retail; after measured manager and table differences, 7,616 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `9,088 + 880 + 0 + 1,024 = 10,992`; the component census isolates 7,616 bytes for A3 |
...
1. **Finish attribution.** Extend the existing owner-level reconciliation with
   ...
   accounted; the 7,616-byte DOS/BIOS remainder still needs safe subranges.
...
3. **Reach component parity or document the irreducible difference.** Target
   ...
   block is 610,112 bytes.
4. **Recover real layout bytes.** Safely relocating the EBDA raises that
   projection to 611,136 bytes. At least another 7,600 bytes must then come from
...
the prerequisite for resolving the 8,624-byte layout row rather than moving it
between labels.
...
The current milestone reproduces 607,744 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 28,048 bytes versus retail's
18,880, a 9,168-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 9,088 bytes |
...
Subtracting those measured components leaves **7,616 bytes** of retained
...
1,700 discardable bytes. The BIOS has 15,920 linked bytes of resident-code
capacity and 20,074 discardable SYSINIT bytes; its possible hardware-selected
resident boundaries range from 8,256 bytes for floppy-only through 12,416 bytes
...
extension. `ENDONEHARD` is exactly 8,352 bytes; the contiguous relocated
121-byte day converter and 5-byte BCD converter establish the exact `2120h` or
8,480-byte resident BIOS boundary.
...
The fixed VC image's grouped pre-MCB payload is 15,424 bytes: the exact
6,944-byte paragraph-rounded DOS gateway plus the exact 8,480-byte selected
...
group-level MCB/gap overhead, these ranges account for the current 15,568-byte
non-component system footprint. Retail's corresponding remainder is 7,952
bytes, producing the already measured 7,616-byte excess. The likely
...
| 5 | Census and relocate eligible DOS low state | Part of the 7,616-byte DOS/BIOS remainder; 6,944 bytes allocated locally | Apply HMA, relocation-safe XMS, then bounded UMB placement according to ownership and lifetime |
| 6 in progress | Compact the selected BIOS resident image | Part of the same 7,616-byte remainder; the fixed hardware path is now exactly 8,480 bytes | Continue map-guided compaction without changing BIOS-visible services |
...
| 2 | Place mutable DOS structures high, then prefer HMA for buffers; DR-DOS 6 measures a 12,800-byte conventional move and documents `HIDOS`/`HIBUFFERS` | The local DOS/BIOS remainder is 7,616 bytes; only 1,216 bytes of UMB advantage may be spent before falling below retail | Classify each owner, then use HMA, relocation-safe XMS, and finally deterministic UMB placement; filesystem, device, redirector, EXEC, A20-off, and rollback paths must pass |
...
inferring layout from component totals. The current 8,624-byte row may contain
...
The current 607,744-byte largest block is 10,992 bytes (1.8%) below retail and
