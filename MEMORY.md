...
10,800-byte gap. Three successive EMM386 paragraphs raise the current result to
607,984 bytes and reduce the gap to 10,752 bytes.
...
| --- | --- | ---: |
| Advantage | EMM386 resident allocation | -80 |
...
| **Total** | **VC largest-block gap** | **10,752** |
...
usable UMB space. The current result is 607,984 bytes, so 10,752 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | -80 bytes | 607,984 bytes |
| HIMEM resident allocation | 1,488 bytes | 609,472 bytes |
| COMMAND resident allocation | 880 bytes | 610,352 bytes |
...
Treat the 10,752-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 10,752 - EMM386 gain - HIMEM gain - COMMAND gain
...
replacement block raises that to 3,392 bytes, still leaving **7,360 bytes**.
The final route must
therefore include at least one of these outcomes:

- recover at least 7,360 bytes from retained DOS/BIOS layout and fragmentation;
- make EMM386, HIMEM, or COMMAND collectively at least 7,408 bytes smaller than
...
5. **In progress:** EMM386 now retains a 444-byte dual-mode `_TEXT` gateway;
...
| 1 | Complete the small EMM386 low gateway backed by locked XMS | Local active EMM386 is 80 bytes below retail after compacting AUTO state selection and its real-mode tail, removing a redundant post-paging TLB flush, saving one complete 386 register frame, and the earlier transition/data work; services, return and A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity handling, and the protected `GoVirtual` continuation execute from locked XMS | create further better-than-retail headroom | Remaining transition, DMA, and fault gateways; inactive `AUTO`; all EMS maps |
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 8,848 bytes more than retail; after measured manager and table differences, 7,456 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `8,848 + 880 + 0 + 1,024 = 10,752`; the component census isolates 7,456 bytes for E1 |
...
| Complete C1 | EMM386 | Maintain byte-range accounting for the current 444-byte low `_TEXT` prefix and 424-byte `_DATA`, including local labels, anonymous gaps, VDATA overlay, discarded loader stack, and paragraph padding | Every retained byte is real-only, dual-mapped, mutable runtime, compatibility state, discarded initialization state, or unresolved |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 444 low bytes precede the
boundary and 17,568 non-low bytes follow it. The report now accounts for
the complete 1,632-byte static low-image address range: a 388-byte real-mode
gateway, 176-byte GDT, 424-byte `_DATA`, 177-byte constants, 10-byte BSS,
13 bytes of alignment, and the 444-byte dual-mode `_TEXT` prefix. The full
...
computed installed allocation to 4,048 bytes, 80 bytes below retail and
matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 607,984 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 27,808 bytes versus retail's
18,880, an 8,928-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 8,848 bytes |
| COMMAND start through VC start | 880 bytes |
| VC start through conventional free block | 0 bytes |
| Conventional ceiling | 1,024 bytes |
| **Total** | **10,752 bytes** |

The pre-COMMAND system span contains EMM386's 80-byte advantage, a 1,488-byte
...
BIOS image. The following system MCB contains 12,544 bytes; enumerated
components occupy 12,432 bytes, leaving 112 bytes. Together with 32 bytes of
...
| 1 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | Active EMM386 is 80 bytes below retail after compacting AUTO state selection and its real-mode tail, removing a redundant post-paging TLB flush, saving one complete 386 register frame, and the earlier transition/data work; services, return/A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity handling, parity-vector state, and the `GoVirtual` continuation are high | Continue toward the smaller owner by redesigning remaining transition, fault, and DMA entries; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
Active EMM386 now occupies 4,048 bytes, 80 below retail. Potential further
...
| EMM386 | 4,048 | 4,128 | -80 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 12,432 | 11,040 | 1,392 |

`MEM` reports 12,544 and 11,168 bytes after each block's arena overhead. Both
...
The current 607,984-byte largest block is 10,752 bytes (1.7%) below retail and
