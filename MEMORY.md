...
594,176 bytes, a 24,560-byte gap.
...
| Open | Larger resident system components | 11,472 bytes |
...
usable UMB space. The current result is 594,176 bytes, so 24,560 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 9,648 bytes | 603,824 bytes |
...
Matching only the three measured component sizes recovers at most 14,848 bytes
...
Treat the 24,560-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 24,560 - EMM386 gain - HIMEM gain - COMMAND gain
...
14,848 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 15,872 bytes, still leaving **8,688 bytes**. The final route must
...
| --- | --- | --- | ---: | --- |
| 1 | Compact EMM386 runtime-sized metadata and alignment | 9,648-byte component excess; initialization state, three tables, LOADALL scratch, and OS/E state reduced or relocated | tens to hundreds of bytes per item | Full `H=`/`A=` ranges and EMS 4.0 formats |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 8,195 low bytes precede the
boundary and 9,874 protected-only bytes follow it. The map exposes 80 symbols
in the retained or dual-mode prefix, 101 in the protected-only suffix, and 65
...
1. complete byte-range accounting inside EMM386's 8,195-byte retained code
   prefix and 1,098-byte `_DATA`, including local labels and alignment;
...
The validated baseline reproduces 594,176 versus 618,736 bytes, the component
...
EMM386 is the largest component opportunity: 13,776 bytes here versus 4,128 in
...
| EMM386 | 13,776 | 4,128 | 9,648 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 22,512 | 11,040 | 11,472 |

`MEM` reports 22,624 and 11,168 bytes after each block's arena overhead. Both
...
The measured system-component excess is 11,472 bytes. The current paired VC
...
The current 594,176-byte largest block is 24,560 bytes (4.0%) below retail and
