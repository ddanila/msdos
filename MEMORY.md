...
and the complete immutable COMMAND catalog payload in HMA leave 602,816 bytes,
a 15,920-byte gap.
...
| **Total** | **VC largest-block gap** | **15,920** |
...
usable UMB space. The current result is 602,816 bytes, so 15,920 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 3,344 bytes | 606,160 bytes |
...
Matching only the three measured component sizes recovers at most 6,816 bytes
...
Treat the 15,920-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 15,920 - EMM386 gain - HIMEM gain - COMMAND gain
...
6,816 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 7,840 bytes, still leaving **8,080 bytes**. The final route must
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `12,912 + 1,984 + 0 + 1,024 = 15,920`; the component census isolates 8,096 bytes for A3 |
...
The current milestone reproduces 602,816 versus 618,736 bytes, the component
...
paired VC capture measures 602,816 bytes in the largest block and a 1,984-byte
...
The current 602,816-byte largest block is 15,920 bytes (2.6%) below retail and
