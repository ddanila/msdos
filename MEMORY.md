...
607,904 bytes, a 10,832-byte gap.
...
is another oversized component. The 7,488-byte DOS/BIOS row is an exact
owner-level remainder. Phase A3 now accounts for every byte of both the
6,846-byte DOS low prefix and the selected 8,448-byte BIOS image. E1 must turn
...
usable UMB space. The current result is 607,904 bytes, so 10,832 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | -32 bytes | 607,904 bytes |
| HIMEM resident allocation | 1,488 bytes | 609,392 bytes |
| COMMAND resident allocation | 880 bytes | 610,272 bytes |
| Layout and conventional ceiling | 8,496 bytes | 618,768 bytes |
...
Treat the 10,832-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 10,832 - EMM386 gain - HIMEM gain - COMMAND gain
...
replacement block raises that to 3,392 bytes, still leaving **7,440 bytes**.
The final route must
therefore include at least one of these outcomes:

- recover at least 7,440 bytes from retained DOS/BIOS layout and fragmentation;
- make EMM386, HIMEM, or COMMAND collectively at least 7,472 bytes smaller than
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 8,928 bytes more than retail; after measured manager and table differences, 7,488 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `8,928 + 880 + 0 + 1,024 = 10,832`; the component census isolates 7,488 bytes for E1 |
| Complete A3 | Measurement | Add reproducible HIMEM, EMM386, COMMAND, DOS, and BIOS resident-range reports from their linker maps | HIMEM, EMM386, COMMAND top-level lifetimes, fixed HMA layout, and every byte of the 6,846-byte DOS low prefix and selected 8,448-byte BIOS image are accounted |
...
The current milestone reproduces 607,904 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 27,888 bytes versus retail's
18,880, a 9,008-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 8,928 bytes |
...
Subtracting those measured components leaves **7,488 bytes** of retained
...
For the current build, `DOS_LOW_GATE_END` is 6,846 linked bytes and consumes
6,848 paragraph-rounded bytes below the HMA. The HMA copy contains 39,488 bytes
...
The fixed VC image's grouped pre-MCB payload is 15,296 bytes: the exact
6,848-byte paragraph-rounded DOS gateway plus the exact 8,448-byte selected
...
group-level MCB/gap overhead, these ranges account for the current 15,440-byte
non-component system footprint. Retail's corresponding remainder is 7,952
bytes, producing the already measured 7,488-byte excess. The likely
...
| 5 in progress | Census and relocate eligible DOS low state | Part of the 7,488-byte DOS/BIOS remainder; 6,848 bytes allocated locally after discarding initialization-only identification data and removing dead table state | Continue the address/lifetime audit, then apply HMA, relocation-safe XMS, and bounded UMB placement |
| 6 in progress | Compact the selected BIOS resident image | Part of the same 7,488-byte remainder; the fixed hardware path is now exactly 8,448 bytes | Continue map-guided compaction without changing BIOS-visible services |
...
| 2 | Place mutable DOS structures high, then prefer HMA for buffers; DR-DOS 6 measures a 12,800-byte conventional move and documents `HIDOS`/`HIBUFFERS` | The local DOS/BIOS remainder is 7,488 bytes; only 1,216 bytes of UMB advantage may be spent before falling below retail | Classify each owner, then use HMA, relocation-safe XMS, and finally deterministic UMB placement; filesystem, device, redirector, EXEC, A20-off, and rollback paths must pass |
...
and dead-state work reduces the current low gateway allocation to 6,848 bytes.
...
The current 607,904-byte largest block is 10,832 bytes (1.8%) below retail and
