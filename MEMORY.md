...
copies its live frames and switches SS before releasing low storage; shrink
failure copies current frames back and switches low before freeing the UMB.
No additional allocation or retained low stack mirror is introduced. EXEC and
reload select the data/stack owner, not the PSP. The redundant reload data
selector is removed. Environment finalization explicitly uses low INIT state;
its former implicit SS-relative reads are invalid with an upper stack.

The paired composition `out/command-upper-data-e0yw4ez3/` measures
**625,408 -> 625,520 conventional**, **48,064 -> 47,936 free UMB**, unchanged
application XMS, and no stranded low hole. The upper allocation grows
**336 -> 464 bytes**; its existing MCB does not grow. COMMAND retains only
**256 PSP + 163 entry/restore-gate + 13 alignment = 432 low bytes**. Its HMA
payload falls **5,846 -> 5,839**, leaving the joint tail at `F30Eh..FFF0h`.
The default COMMAND binary is unchanged.

Local qualification passes: destructive pipeline/reload and environment checks
on that composition; allocation/shrink rejection with no UMB leaks
(`out/command-upper-failure-mqeawpl2/`); recoverable policy rejection and explicit
stop before AUTOEXEC on persistent failure (`...-y7k7wenl/`); and A20-off INT 2Eh
across manager mode requests (`out/command-upper-int2e-slks8vj7/`). The INT 2Eh
probe verifies SS:SP belongs to the parent shell's exact upper MCB, with wrong-stack
and omitted-A20-disable negative controls. Rejected placement retains **880 low
bytes**, not the released layout. The linked stack/environment regressions are
in `test_command_upper_stack.py`; broader asynchronous/286/DOS-low qualification
and final BIOS placement remain open. This completes one remaining owner, not
the joint delivery gate.
...
`DISKSECTOR` before each format attempt. Persistent R/N state remains separate:
regular I/O and media checks may overwrite `DISKSECTOR`. No second low copy,
new allocation or transport protocol survives. BIOS allocation rounding turns
126 retired bytes into **128 conventional bytes**; its high payload grows
from 7,986 to 7,987 bytes.

Paired VC/MEM evidence: `out/emm-mode-guard-8a_tib1b/results.json`, compared
with `out/emm-mode-guard-um9x123d/input.img`. Local checks pass: all 54 BIOS
unit/build tests (including all 63 initial pairs), 1.44/2.88 MB floppy formatting
...
shared high payload. `BDSPROC.INC` supplies both the high owner and its cold
fallback, now linked outside the retained boundary. The low `INT2F_DISK` filter
keeps unrelated chains and the E704h boot-chain rebase HMA-independent: E705h
A20 restoration must reach HIMEM without recursively entering the high service.
Owned calls enter high only after activation and A20 restoration. Request
completion reuses the existing low `DSK$IN` entry; BDS roots, records and request
state retain their public low addresses. Normal IO.SYS remains byte-identical.

`out/bios-mux-retirement-rr2vha88/` compares unchanged startup, kernel, managers,
COMMAND and VC with only IO.SYS replaced: **624,544 -> 624,624 conventional**,
unchanged **48,416 UMB / 6,798,336 application XMS**. BIOS falls **2,800 -> 2,720**
low bytes; its high payload grows **7,853 -> 7,986**. Public probes cover installed/
reserved queries, returned BDS roots, foreign and shared-physical BDS insertion
...
(`out/stack-pool-retirement-pxxm0p9s/`). Hardware reset does not qualify software INT 19h.

Released cold storage may already belong to another allocation when a program
runs. Check its release boundary and high binding, not persistence of the boot
poison fill. BIOS reboot/swap paths, other asynchronous cases and the complete
BIOS/COMMAND state contract remain open; this does not finish joint placement.
...
into the existing upper pool would leave **47,328 free UMB bytes**, 560 below
...
fallback qualification against the same 3,623-byte HMA budget; do not resume
...
| --- | ---: | ---: | ---: | ---: |
| One hard disk | 500 | 300 | 132 / 33 | 299 |
| Two hard disks | 600 | 400 | 132 / 66 | 266 |

Every BDS is 100 bytes; every DPB is 33. The two initial DPBs already belong
to DOS and are excluded from the BIOS figures. The first hard-disk census row
...
the existing upper-table moves run later. A late graph move therefore also
needs a second low-layout release transaction. In addition, `SETDRIVE_FIELD`
in MSDSKHIG.INC and `IOCTL$GETOWN` in MSIOCTL.INC ignore START_BDS's segment
word for the first record. Other traversals follow far links. UMB placement
requires fixing those consumers, preserving cached public pointers and testing
A20-off/EMS access; fitting under the 1,792-byte free-UMB margin is not enough.
The census reads physical conventional RAM and intentionally rejects UMB/HMA
...
The current shared HMA budget is **40,272 DOS + 7,986 BIOS + 7,988 buffers +
5,635 COMMAND = 61,881 bytes**, leaving **3,623 bytes** at `F1C9h..FFF0h`.
...
Completing dispatch retirement recovers another **160 bytes**: the cold
decoder joins the disposable tables, and the high far tables directly select
...
owners remain in scope; its current shared budget is the 3,623-byte remainder.
