...
continuation, protected stack-selector conversion, and transient command-mode
client split leave 606,992 bytes, an 11,744-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 272 |
...
| **Total** | **VC largest-block gap** | **11,744** |
...
usable UMB space. The current result is 606,992 bytes, so 11,744 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 272 bytes | 607,264 bytes |
...
Matching only the three measured component sizes recovers at most 2,640 bytes
...
Treat the 11,744-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 11,744 - EMM386 gain - HIMEM gain - COMMAND gain
...
2,640 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 3,664 bytes, still leaving **8,080 bytes**. The final route must
...
5. **In progress:** EMM386 now retains a 587-byte dual-mode `_TEXT` gateway;
...
| 1 | Complete the small EMM386 low gateway backed by locked XMS | Local EMM386 now costs 272 bytes more than retail; services, return and A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity handling, and the protected `GoVirtual` continuation execute from locked XMS; command-mode client code is nonresident | hundreds of bytes locally; better-than-retail remains possible | Remaining transition, DMA, and fault gateways; inactive `AUTO`; all EMS maps |
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 9,840 bytes more than retail; after measured manager and table differences, 8,096 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `9,840 + 880 + 0 + 1,024 = 11,744`; the component census isolates 8,096 bytes for A3 |
...
boundary and 17,244 non-low bytes follow it. The report now accounts for
the complete 2,011-byte static low-image address range: a 426-byte real-mode
gateway, 272-byte GDT, 636-byte `_DATA`, 177-byte constants, 10-byte BSS,
...
configuration uses 5 alignment bytes, bringing the computed installed
allocation to 4,400 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 606,992 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 28,800 bytes versus retail's
18,880, a 9,920-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 9,840 bytes |
| COMMAND start through VC start | 880 bytes |
| VC start through conventional free block | 0 bytes |
| Conventional ceiling | 1,024 bytes |
| **Total** | **11,744 bytes** |

The pre-COMMAND system span contains the 272-byte EMM386 excess, 1,488-byte
...
| 1 | Complete the EMM386 low gateway | 272-byte excess over retail; services, return and A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity, and the `GoVirtual` continuation execute from locked XMS; command-mode client code is nonresident | Redesign the remaining transition, DMA, and fault gateways; preserve inactive `AUTO` and every EMS map |
| 1 | Compact EMM386 metadata while changing that boundary | The loader stack is discarded, duplicate PTE offsets and inverse segment index are gone, protected entry/services/helpers are high, the two-function control gateway is table-free, `NOHIMEM` hooks are absent, and fixed selector/parity operations are compact; remaining work is included in the 272-byte excess | Take independently testable descriptor, VDATA, table, and alignment wins without delaying the gateway design |
...
| 1 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is now 272 bytes above retail after moving services, return/A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity handling, and the `GoVirtual` continuation high, and making command-mode client code nonresident | Continue toward the smaller owner by redesigning remaining transition, fault, and DMA entries; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 now occupies 4,400 bytes here versus 4,128 in retail. Potential further
...
CONFIG.SYS installation retains only the device header and runtime gateway. The public
command behavior is unchanged and the existing parser was already non-low.
...
remaining 272-byte component excess and potential better-than-retail gateway
...
| EMM386 | 4,400 | 4,128 | 272 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 12,784 | 11,040 | 1,744 |

`MEM` reports 12,896 and 11,168 bytes after each block's arena overhead. Both
...
The current 606,992-byte largest block is 11,744 bytes (1.9%) below retail and
