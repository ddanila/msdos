...
leave 604,976 bytes, a 13,760-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 2,288 |
...
| **Total** | **VC largest-block gap** | **13,760** |
...
usable UMB space. The current result is 604,976 bytes, so 13,760 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 2,288 bytes | 607,264 bytes |
...
Matching only the three measured component sizes recovers at most 5,280 bytes
...
Treat the 13,760-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 13,760 - EMM386 gain - HIMEM gain - COMMAND gain
...
4,736 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 5,760 bytes, still leaving **8,080 bytes**. The final route must
...
| 1 | Build a small EMM386 low gateway backed by locked XMS | Local EMM386 costs 2,288 bytes more than retail; protected entry, core queries, handle metadata and attributes, hardware information, mapping services, and allocation mutations are high | several kilobytes | Real/virtual transitions, inactive `AUTO`, DMA, faults, and all EMS maps |
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 11,856 bytes more than retail; after measured manager and table differences, 8,096 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `11,856 + 880 + 0 + 1,024 = 13,760`; the component census isolates 8,096 bytes for A3 |
...
| Complete C1 | EMM386 | Produce byte-range accounting inside the current 3,276-byte low `_TEXT` prefix and 634-byte `_DATA`, including local labels, anonymous gaps, VDATA overlay, discarded loader stack, and paragraph padding | Every retained byte is real-only, dual-mapped, mutable runtime, compatibility state, discarded initialization state, or unresolved |
...
| D1 in progress | COMMAND | Refine the generated resident byte-range census from lifetime classes to symbol/module ownership | The checked report anchors every permanent-low range; the first code tranche moves 76 bytes behind bounded HMA gateways and reduces the high shell by one paragraph |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 2,487 low bytes precede the
boundary and 15,316 protected-only bytes follow it. The report now accounts for
the complete 4,029-byte static low-image address range: a 442-byte real-mode
gateway, 272-byte GDT, 634-byte `_DATA`, 177-byte constants, 10-byte BSS,
7 bytes of alignment, and the 2,487-byte dual-mode `_TEXT` prefix. The full
...
configuration uses 3 bytes, bringing the computed installed allocation to
6,416 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 604,976 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 30,816 bytes versus retail's
18,880, an 11,936-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 11,856 bytes |
| COMMAND start through VC start | 880 bytes |
| VC start through conventional free block | 0 bytes |
| Conventional ceiling | 1,024 bytes |
| **Total** | **13,760 bytes** |

The pre-COMMAND system span contains the 2,288-byte EMM386 excess, 1,488-byte
...
components occupy 15,632 of 15,744 bytes, leaving 112 bytes. Together with the
...
| 1 | Extend the EMM386 low gateway | 2,288-byte excess over retail; protected entry, core register queries, handle metadata and attributes, hardware information, mapping services, and allocation mutations execute from locked XMS | Move the next coherent service family while leaving unsupported aliases, transition, DMA, fault, far-buffer, and return gateways low; preserve every EMS map |
| 1 | Compact EMM386 metadata while changing that boundary | The loader stack is discarded, duplicate PTE offsets and inverse segment index are gone, protected entry/allocation/mapping/metadata/information/query code is high, and duplicate null device requests are gone; remaining work is included in the 2,288-byte excess | Take independently testable descriptor, VDATA, table, and alignment wins without delaying the gateway design |
...
| 2 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is 2,288 bytes above retail after discarding its loader stack, deriving PTE offsets, eliminating the inverse segment index, collapsing null device requests, and moving protected entry/allocation/mapping/metadata/information/query code high | Continue splitting transition, fault, DMA, far-buffer, and inactive-`AUTO` entries from protected/XMS state; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 is the largest component opportunity: 6,416 bytes here versus 4,128 in
...
Functions 40h, 41h, 42h, 46h, and 4Bh now form a protected core-query object.
The protected bitmap was rebased to 40h, with function 5Ch included because its
warm-boot contract aliases the relocated status implementation. Unsupported
49h/4Ah and the far-buffer 4Ch/4Dh pair remain in the low dispatcher; moving
4Ch/4Dh did not survive the complete EMS lifecycle and is not retained. The
stable split reduces EMM386 from 6,496 to 6,416 bytes and raises the paired VC
block to 604,976, leaving a 13,760-byte retail gap. Inactive `AUTO` explicitly
exercises status, page counts, version, and handle queries. The full EMS,
frame, load-option, UMB/XMS transaction, HIMEM lifecycle, `/NUMHANDLES=24..32`,
and 286/386/486 hardware gates pass.
...
and return lifetimes are traced under the complete EMS sequence.
...
2,288-byte low-gateway opportunity. Revisit COMMAND only as a coherent
...
| EMM386 | 6,416 | 4,128 | 2,288 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 14,800 | 11,040 | 3,760 |

`MEM` reports 14,912 and 11,168 bytes after each block's arena overhead. Both
...
The current 604,976-byte largest block is 13,760 bytes (2.2%) below retail and
