...
leave 604,352 bytes, a 14,384-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 2,912 |
...
| **Total** | **VC largest-block gap** | **14,384** |
...
usable UMB space. The current result is 604,352 bytes, so 14,384 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 2,912 bytes | 607,264 bytes |
...
Treat the 14,384-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 14,384 - EMM386 gain - HIMEM gain - COMMAND gain
...
| 1 | Build a small EMM386 low gateway backed by locked XMS | Local EMM386 costs 2,912 bytes more than retail; protected entry, mapping services, and allocation mutations are high | several kilobytes | Real/virtual transitions, inactive `AUTO`, DMA, faults, and all EMS maps |
...
| 3 | Classify and relocate eligible DOS low state | The local system span costs 12,480 bytes more than retail and 20,752 more than DR-DOS | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `12,480 + 880 + 0 + 1,024 = 14,384`; the component census isolates 8,096 bytes for A3 |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 3,110 low bytes precede the
boundary and 14,688 protected-only bytes follow it. The report now accounts for
the complete 4,652-byte static low-image address range: a 442-byte real-mode
gateway, 272-byte GDT, 634-byte `_DATA`, 177-byte constants, 10-byte BSS,
7 bytes of alignment, and the 3,110-byte dual-mode `_TEXT` prefix. The full
...
configuration uses 4 bytes, bringing the computed installed allocation to
7,040 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 604,352 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 31,440 bytes versus retail's
18,880, a 12,560-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 12,480 bytes |
| COMMAND start through VC start | 880 bytes |
| VC start through conventional free block | 0 bytes |
| Conventional ceiling | 1,024 bytes |
| **Total** | **14,384 bytes** |

The pre-COMMAND system span contains the 2,912-byte EMM386 excess, 1,488-byte
...
| 1 | Extend the EMM386 low gateway | 2,912-byte excess over retail; protected entry, mapping services, and allocation mutations execute from locked XMS | Move the next coherent service family while leaving save-map/query, transition, DMA, fault, inactive-`AUTO`, and return gateways low; preserve every EMS map |
| 1 | Compact EMM386 metadata while changing that boundary | The loader stack is discarded, duplicate PTE offsets and inverse segment index are gone, protected entry/allocation/mapping code is high, and duplicate null request entries are gone; remaining work is included in the 2,912-byte excess | Take independently testable descriptor, VDATA, table, and alignment wins without delaying the gateway design |
...
| 2 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is 2,912 bytes above retail after discarding its loader stack, deriving PTE offsets, eliminating the inverse segment index, collapsing null device requests, and moving protected entry/allocation/mapping code high | Continue splitting query, transition, fault, DMA, and inactive-`AUTO` entries from protected/XMS state; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 is the largest component opportunity: 7,040 bytes here versus 4,128 in
...
2,912-byte low-gateway opportunity. Revisit COMMAND only as a coherent
...
paired VC capture measures 604,352 bytes in the largest block and an 880-byte
...
| EMM386 | 7,040 | 4,128 | 2,912 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 15,424 | 11,040 | 4,384 |

`MEM` reports 15,536 and 11,168 bytes after each block's arena overhead. Both
...
The current 604,352-byte largest block is 14,384 bytes (2.3%) below retail and
