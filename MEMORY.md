...
and the shared EMM386 dispatch table leave 602,912 bytes, a 15,824-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 3,248 |
...
| **Total** | **VC largest-block gap** | **15,824** |
...
usable UMB space. The current result is 602,912 bytes, so 15,824 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 3,248 bytes | 606,160 bytes |
...
Matching only the three measured component sizes recovers at most 6,720 bytes
...
Treat the 15,824-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 15,824 - EMM386 gain - HIMEM gain - COMMAND gain
...
6,720 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 7,744 bytes, still leaving **8,080 bytes**. The final route must
...
| 2 | Build a small EMM386 low gateway backed by locked XMS | Local EMM386 costs 3,248 bytes more than retail; protected mapping services and allocation mutations are high | several kilobytes | Real/virtual transitions, inactive `AUTO`, DMA, faults, and all EMS maps |
...
| 3 | Classify and relocate eligible DOS low state | The local system span costs 12,816 bytes more than retail and 21,088 more than DR-DOS | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `12,816 + 1,984 + 0 + 1,024 = 15,824`; the component census isolates 8,096 bytes for A3 |
...
| Complete C1 | EMM386 | Produce byte-range accounting inside the current 3,246-byte low `_TEXT` prefix and 798-byte `_DATA`, including local labels, anonymous gaps, VDATA overlay, discarded loader stack, and paragraph padding | Every retained byte is real-only, dual-mapped, mutable runtime, compatibility state, discarded initialization state, or unresolved |
...
the complete 4,984-byte static low-image address range: a 474-byte real-mode
...
configuration uses 8 bytes, bringing the computed installed
allocation to 7,376 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 602,912 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 31,776 bytes versus retail's
18,880, a 12,896-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 12,816 bytes |
...
The pre-COMMAND system span contains the 3,248-byte EMM386 excess, 1,488-byte
...
components occupy 15,760 of 15,872 bytes, leaving 112 bytes. Together with the
...
| 3 | Extend the EMM386 low gateway | 3,248-byte excess over retail; protected mapping and allocation services execute from locked XMS | Move the next coherent service family while leaving save-map/query, transition, DMA, fault, inactive-`AUTO`, and return gateways low; preserve every EMS map |
| 4 | Compact EMM386 metadata while changing that boundary | The loader stack is discarded and mapping/allocation code is high; remaining work is included in the 3,248-byte excess | Take independently testable descriptor, VDATA, table, and alignment wins without delaying the gateway design |
...
| 2 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is 3,248 bytes above retail after discarding its loader stack and moving allocation and mapping services high | Continue splitting query, transition, fault, DMA, and inactive-`AUTO` entries from protected/XMS state; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 is the largest component opportunity: 7,376 bytes here versus 4,128 in
...
paired VC capture measures 602,912 bytes in the largest block and a 1,984-byte
...
| EMM386 | 7,376 | 4,128 | 3,248 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 15,760 | 11,040 | 4,720 |

`MEM` reports 15,872 and 11,168 bytes after each block's arena overhead. Both
...
The current 602,912-byte largest block is 15,824 bytes (2.6%) below retail and
