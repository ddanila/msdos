...
is another oversized component. The 6,016-byte DOS/BIOS row is an exact
owner-level remainder. Phase A3 now accounts for every byte of both the
5,392-byte DOS low prefix and the selected 8,432-byte BIOS image. E1 must turn
...
usable UMB space. The current result is 609,536 bytes, so 9,200 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | -192 bytes | 609,536 bytes |
| HIMEM resident allocation | 1,488 bytes | 611,024 bytes |
| COMMAND resident allocation | 880 bytes | 611,904 bytes |
| Layout and conventional ceiling | 6,832 bytes | 618,736 bytes |
...
Treat the 9,200-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 9,200 - EMM386 gain - HIMEM gain - COMMAND gain
...
replacement block raises that to 3,392 bytes, still leaving **5,808 bytes**.
The final route must
therefore include at least one of these outcomes:

- recover at least 5,808 bytes from retained DOS/BIOS layout and fragmentation;
- make EMM386, HIMEM, or COMMAND collectively at least 5,856 bytes smaller than
...
| 3 | Classify and relocate eligible DOS low state | The dispatcher and 652-byte absolute/system/INT 2Fh tranche are high; the local system-to-COMMAND span now costs 7,296 bytes more than retail and leaves 6,016 bytes of measured DOS/BIOS remainder | low kilobytes | Classify the remaining tables and mutable state; some low addresses are ABI/BIOS fixed |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `7,296 + 880 + 0 + 1,024 = 9,200`; the component census isolates 6,016 bytes for E1 |
| Complete A3 | Measurement | Add reproducible HIMEM, EMM386, COMMAND, DOS, and BIOS resident-range reports from their linker maps | HIMEM, EMM386, COMMAND top-level lifetimes, fixed HMA layout, and every byte of the 5,392-byte DOS low prefix and selected 8,432-byte BIOS image are accounted |
...
The current milestone reproduces 609,536 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 26,256 bytes versus retail's
18,880, a 7,376-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 7,296 bytes |
...
Subtracting those measured components leaves **6,016 bytes** of retained
...
1,795 discardable bytes. The BIOS has 15,872 linked bytes of resident-code
...
the exact `20F0h` or 8,432-byte resident BIOS boundary.
...
The selected BIOS census likewise covers all 8,432 bytes. Its largest ranges
are 1,787 bytes of generic disk IOCTL/INT 2F services, 1,575 bytes of core data
...
The fixed VC image's grouped pre-MCB payload is 13,824 bytes: the exact
5,392-byte paragraph-rounded DOS gateway plus the exact 8,432-byte selected
...
group-level MCB/gap overhead, these ranges account for the current 13,968-byte
non-component system footprint. Retail's corresponding remainder is 7,952
bytes, producing the already measured 6,016-byte excess. The likely
...
| 2 | Place mutable DOS structures high, then prefer HMA for buffers; DR-DOS 6 measures a 12,800-byte conventional move and documents `HIDOS`/`HIBUFFERS` | The dispatcher and absolute/system/INT 2Fh tranche are high and the local DOS/BIOS remainder is 6,016 bytes; only 1,216 bytes of UMB advantage may be spent before falling below retail | Classify each remaining owner, then use HMA, relocation-safe XMS, and finally deterministic UMB placement; filesystem, device, redirector, EXEC, A20-off, and rollback paths must pass |
...
The current 609,536-byte largest block is 9,200 bytes (1.5%) below retail and
