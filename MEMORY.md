...
current result to 607,920 bytes, a 10,816-byte gap.
...
is another oversized component. The 7,472-byte DOS/BIOS row is an exact
owner-level remainder. Phase A3 now accounts for every byte of both the
6,820-byte DOS low prefix and the selected 8,448-byte BIOS image. E1 must turn
...
usable UMB space. The current result is 607,920 bytes, so 10,816 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | -32 bytes | 607,920 bytes |
| HIMEM resident allocation | 1,488 bytes | 609,408 bytes |
| COMMAND resident allocation | 880 bytes | 610,288 bytes |
| Layout and conventional ceiling | 8,480 bytes | 618,768 bytes |
...
Treat the 10,816-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 10,816 - EMM386 gain - HIMEM gain - COMMAND gain
...
replacement block raises that to 3,392 bytes, still leaving **7,424 bytes**.
The final route must
therefore include at least one of these outcomes:

- recover at least 7,424 bytes from retained DOS/BIOS layout and fragmentation;
- make EMM386, HIMEM, or COMMAND collectively at least 7,456 bytes smaller than
...
3. **Complete:** the top-level COMMAND census and exact HMA
   ...
   18,060 bytes of initially free DOS-owned HMA tail storage; continue the
   symbol-level
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 8,912 bytes more than retail; after measured manager and table differences, 7,472 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `8,912 + 880 + 0 + 1,024 = 10,816`; the component census isolates 7,472 bytes for E1 |
| Complete A3 | Measurement | Add reproducible HIMEM, EMM386, COMMAND, DOS, and BIOS resident-range reports from their linker maps | HIMEM, EMM386, COMMAND top-level lifetimes, fixed HMA layout, and every byte of the 6,820-byte DOS low prefix and selected 8,448-byte BIOS image are accounted |
...
The current milestone reproduces 607,920 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 27,872 bytes versus retail's
18,880, an 8,992-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 8,912 bytes |
...
Subtracting those measured components leaves **7,472 bytes** of retained
...
For the current build, `DOS_LOW_GATE_END` is 6,820 linked bytes and consumes
6,832 paragraph-rounded bytes below the HMA. The HMA copy contains 39,456 bytes
...
The fixed VC image's grouped pre-MCB payload is 15,280 bytes: the exact
6,832-byte paragraph-rounded DOS gateway plus the exact 8,448-byte selected
...
group-level MCB/gap overhead, these ranges account for the current 15,424-byte
non-component system footprint. Retail's corresponding remainder is 7,952
bytes, producing the already measured 7,472-byte excess. The likely
...
| 5 in progress | Census and relocate eligible DOS low state | Part of the 7,472-byte DOS/BIOS remainder; 6,832 bytes allocated locally after discarding initialization-only data, removing dead state, and overlaying EXEC's MZ header | Continue the address/lifetime audit, then apply HMA, relocation-safe XMS, and bounded UMB placement |
| 6 in progress | Compact the selected BIOS resident image | Part of the same 7,472-byte remainder; the fixed hardware path is now exactly 8,448 bytes | Continue map-guided compaction without changing BIOS-visible services |
...
1. **Move COMMAND cold state high or transient.** DR-DOS 6 proves a shell can
   ...
   span. Our HMA DOS image is 39,456 bytes, so a similarly sized experiment fits
...
occupies `0010h..9A30h` (39,456 bytes). With the fixed 15-buffer, 512-byte-sector
configuration, the hash and slots occupy `9A30h..B964h` (7,988 bytes), leaving
`B964h..FFF0h` (18,060 bytes) of initially unassigned but still DOS-owned space
...
dead-state, and shared-workspace work reduces the current low gateway allocation
to 6,832 bytes.
...
The current 607,920-byte largest block is 10,816 bytes (1.7%) below retail and
