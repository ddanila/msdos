...
lists leave 607,184 bytes, an 11,552-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 80 |
...
| **Total** | **VC largest-block gap** | **11,552** |
...
usable UMB space. The current result is 607,184 bytes, so 11,552 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 80 bytes | 607,264 bytes |
...
Matching only the three measured component sizes recovers at most 2,448 bytes
...
Treat the 11,552-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 11,552 - EMM386 gain - HIMEM gain - COMMAND gain
...
2,448 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 3,472 bytes, still leaving **8,080 bytes**. The final route must
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 9,648 bytes more than retail; after measured manager and table differences, 8,096 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `9,648 + 880 + 0 + 1,024 = 11,552`; the component census isolates 8,096 bytes for A3 |
...
| Complete C1 | EMM386 | Maintain byte-range accounting for the current 483-byte low `_TEXT` prefix and 454-byte `_DATA`, including local labels, anonymous gaps, VDATA overlay, discarded loader stack, and paragraph padding | Every retained byte is real-only, dual-mapped, mutable runtime, compatibility state, discarded initialization state, or unresolved |
...
boundary and 17,403 non-low bytes follow it. The report now accounts for
the complete 1,781-byte static low-image address range: a 426-byte real-mode
gateway, 224-byte GDT, 454-byte `_DATA`, 177-byte constants, 10-byte BSS,
...
configuration uses 3 alignment bytes, bringing the computed installed
allocation to 4,208 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 607,184 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 28,608 bytes versus retail's
18,880, a 9,728-byte difference that includes the memory managers and retained
...
| 1 | Complete the EMM386 low gateway | 80-byte excess over retail; services, return and A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity, and the `GoVirtual` continuation execute from locked XMS; command-mode client code and debugger descriptors are nonresident | Redesign the remaining transition, DMA, and fault gateways; preserve inactive `AUTO` and every EMS map |
| 1 | Compact EMM386 metadata while changing that boundary | The loader stack is discarded, duplicate PTE offsets and inverse segment index are gone, physical-window segments, public `Pn=` identifiers, and the DMA page list are runtime-sized, protected entry/services/helpers are high, the two-function control gateway is table-free, the production GDT omits six debugger descriptors, `NOHIMEM` hooks are absent, and fixed selector/parity operations are compact; remaining work is included in the 80-byte excess | Take independently testable descriptor, VDATA, table, and alignment wins without delaying the gateway design |
...
| 1 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is now 80 bytes above retail after moving services, return/A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity handling, and the `GoVirtual` continuation high, making command-mode client code and debugger descriptors nonresident, and sizing physical-window IDs, indexes, and DMA pages at runtime | Continue toward the smaller owner by redesigning remaining transition, fault, and DMA entries; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 now occupies 4,208 bytes here versus 4,128 in retail. Potential further
...
| EMM386 | 4,208 | 4,128 | 80 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 12,592 | 11,040 | 1,552 |

`MEM` reports 12,704 and 11,168 bytes after each block's arena overhead. Both
...
The current 607,184-byte largest block is 11,552 bytes (1.9%) below retail and
