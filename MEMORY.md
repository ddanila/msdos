...
lists, protected parity-vector state, and byte-bounded counters leave 607,216
bytes, an 11,520-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 48 |
...
| **Total** | **VC largest-block gap** | **11,520** |
...
usable UMB space. The current result is 607,216 bytes, so 11,520 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 48 bytes | 607,264 bytes |
...
Matching only the three measured component sizes recovers at most 2,416 bytes
...
Treat the 11,520-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 11,520 - EMM386 gain - HIMEM gain - COMMAND gain
...
2,416 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 3,440 bytes, still leaving **8,080 bytes**. The final route must
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 9,616 bytes more than retail; after measured manager and table differences, 8,096 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `9,616 + 880 + 0 + 1,024 = 11,520`; the component census isolates 8,096 bytes for A3 |
...
boundary and 17,569 non-low bytes follow it. The report now accounts for
the complete 1,751-byte static low-image address range: a 426-byte real-mode
gateway, 224-byte GDT, 424-byte `_DATA`, 177-byte constants, 10-byte BSS,
...
allocation to 4,176 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 607,216 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 28,576 bytes versus retail's
18,880, a 9,696-byte difference that includes the memory managers and retained
...
| 1 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is now 48 bytes above retail after moving services, return/A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity handling, parity-vector state, and the `GoVirtual` continuation high, making command-mode client code and debugger descriptors nonresident, sizing option-dependent arrays at runtime, and narrowing bounded counters | Continue toward the smaller owner by redesigning remaining transition, fault, and DMA entries; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 now occupies 4,176 bytes here versus 4,128 in retail. Potential further
...
| EMM386 | 4,176 | 4,128 | 48 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 12,560 | 11,040 | 1,520 |

`MEM` reports 12,672 and 11,168 bytes after each block's arena overhead. Both
...
The current 607,216-byte largest block is 11,520 bytes (1.9%) below retail and
