...
594,464 bytes, a 24,272-byte gap.
...
| Open | Larger resident system components | 11,184 bytes |
...
usable UMB space. The current result is 594,464 bytes, so 24,272 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 9,536 bytes | 604,000 bytes |
| HIMEM resident allocation | 1,664 bytes | 605,664 bytes |
| COMMAND resident allocation | 3,360 bytes | 609,024 bytes |
| Layout and conventional ceiling | 9,712 bytes | 618,736 bytes |

Matching only the three measured component sizes recovers at most 14,560 bytes
...
Treat the 24,272-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 24,272 - EMM386 gain - HIMEM gain - COMMAND gain
...
14,560 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 15,584 bytes, still leaving **8,688 bytes**. The final route must
...
| 1 | Remove remaining HIMEM init-only state and padding | 1,664-byte component excess; resident break is explicit | tens to hundreds of bytes | 128 handles, 32 UMB extents, all A20 backends |
...
1. **Finish attribution.** Extend the capture with retained boundaries from the
   ...
   installed components, and the first-free difference `0E9Dh` versus `08F0h`.
...
The validated baseline reproduces 594,464 versus 618,736 bytes, the component
...
HIMEM occupies 2,768 bytes versus retail's 1,104. Audit its resident break at
...
| Total | 22,224 | 11,040 | 11,184 |

`MEM` reports 22,336 and 11,168 bytes after each block's arena overhead. Both
...
The measured system-component excess is 11,184 bytes. The current paired VC
...
- investigate the remaining 1,664-byte HIMEM excess without reducing supported
  ...
  the normal HIMEM allocation by 352 bytes;
...
The current 594,464-byte largest block is 24,272 bytes (3.9%) below retail and
