...
the protected OEM mapping/parity boundary leave 606,416 bytes, a 12,320-byte
gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 848 |
...
| **Total** | **VC largest-block gap** | **12,320** |
...
usable UMB space. The current result is 606,416 bytes, so 12,320 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 848 bytes | 607,264 bytes |
...
Matching only the three measured component sizes recovers at most 3,216 bytes
...
Treat the 12,320-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 12,320 - EMM386 gain - HIMEM gain - COMMAND gain
...
3,216 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 4,240 bytes, still leaving **8,080 bytes**. The final route must
...
| 1 | Build a small EMM386 low gateway backed by locked XMS | Local EMM386 costs 848 bytes more than retail; services, return and A20/NMI trap handling, protected `RetReal`, and OEM mapping/parity handling execute from locked XMS | several kilobytes | Real/virtual transitions, inactive `AUTO`, DMA, faults, and all EMS maps |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `10,416 + 880 + 0 + 1,024 = 12,320`; the component census isolates 8,096 bytes for A3 |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 1,043 low bytes precede the
boundary and 16,672 protected-only bytes follow it. The report now accounts for
the complete 2,587-byte static low-image address range: a 442-byte real-mode
gateway, 272-byte GDT, 636-byte `_DATA`, 177-byte constants, 10-byte BSS,
7 bytes of alignment, and the 1,043-byte dual-mode `_TEXT` prefix. The full
...
allocation to 4,976 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 606,416 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 29,376 bytes versus retail's
18,880, a 10,496-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 10,416 bytes |
| COMMAND start through VC start | 880 bytes |
| VC start through conventional free block | 0 bytes |
| Conventional ceiling | 1,024 bytes |
| **Total** | **12,320 bytes** |

The pre-COMMAND system span contains the 848-byte EMM386 excess, 1,488-byte
...
| 1 | Extend the EMM386 low gateway | 960-byte excess over retail; services, return and A20/NMI trap handling, and protected `RetReal` execute from locked XMS | Treat OEM mapping/parity handling as one coupled EMS boundary, then redesign the remaining transition, DMA, fault, and return gateways; preserve every EMS map |
| 1 | Compact EMM386 metadata while changing that boundary | The loader stack is discarded, duplicate PTE offsets and inverse segment index are gone, protected entry/services/helpers are high, `NOHIMEM` hooks are absent, and fixed selector/parity operations are compact; remaining work is included in the 960-byte excess | Take independently testable descriptor, VDATA, table, and alignment wins without delaying the gateway design |
...
| 1 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is now 848 bytes above retail after moving services, return/A20/NMI trap handling, protected `RetReal`, and OEM mapping/parity handling high | Continue toward the smaller 800-byte owner by redesigning active transition, fault, and DMA entries; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 is the largest component opportunity: 4,976 bytes here versus 4,128 in
...
960-byte low-gateway opportunity. Revisit COMMAND only as a coherent
...
| EMM386 | 4,976 | 4,128 | 848 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 13,360 | 11,040 | 2,320 |

`MEM` reports 13,472 and 11,168 bytes after each block's arena overhead. Both
...
The current 606,416-byte largest block is 12,320 bytes (2.0%) below retail and
