...
594,592 bytes, a 24,144-byte gap.
...
| Open | HIMEM resident excess | 1,536 |
...
| **Total** | **VC largest-block gap** | **24,144** |
...
usable UMB space. The current result is 594,592 bytes, so 24,144 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 9,536 bytes | 604,128 bytes |
| HIMEM resident allocation | 1,536 bytes | 605,664 bytes |
| COMMAND resident allocation | 3,360 bytes | 609,024 bytes |
| Layout and conventional ceiling | 9,712 bytes | 618,736 bytes |

Matching only the three measured component sizes recovers at most 14,432 bytes
...
Treat the 24,144-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 24,144 - EMM386 gain - HIMEM gain - COMMAND gain
...
14,432 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 15,456 bytes, still leaving **8,688 bytes**. The final route must
...
| 1 | Remove remaining HIMEM init-only state and padding | 1,536-byte component excess; resident break is explicit | tens to hundreds of bytes | 128 handles, 32 UMB extents, all A20 backends |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `19,760 + 3,360 + 0 + 1,024 = 24,144`; the component census isolates 8,704 bytes for A3 |
...
The validated baseline reproduces 594,592 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 38,720 bytes versus retail's
18,880, a 19,840-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 19,760 bytes |
...
The pre-COMMAND system span contains the 9,536-byte EMM386 excess, 1,536-byte
...
components occupy 22,096 of 22,208 bytes, leaving 112 bytes. Together with the
...
| ---: | --- | ---: | --- |
| 1 | Finish HIMEM code, data, and tail compaction | 1,536-byte excess over retail | Cross resident paragraph boundaries while preserving the full `/NUMHANDLES=` matrix, XMS 2/3 errors, A20, HMA, UMB rollback, moves, locks, and warm reboot |
| 2 | Compact EMM386 low data, tables, descriptors, stack, VDATA, and alignment | 9,536-byte excess over retail | Complete byte-range ownership inside the measured low `_TEXT` and `_DATA`, then take independently testable paragraph wins |
| 3 | Relocate EMM386 protected-only code and immutable state | Included in that 9,536-byte excess | Extend the locked XMS image while leaving explicit real-mode, transition, DMA, fault, inactive-`AUTO`, and return gateways low |
| 4 | Census and shrink the DOS low prefix | Part of the 8,704-byte DOS/BIOS remainder; 7,600 bytes allocated locally | Classify every constant, mutable range, table, and gateway; move only HMA-safe ownership and consolidate bounce/work areas |
| 5 | Select and compact the BIOS resident boundary | Part of the same 8,704-byte remainder; 8,912 bytes including inferred boundary/alignment | Identify the exact hardware-selected end, remove unused legacy blocks where configuration permits, and compact without changing BIOS-visible services |
| 6 | Reduce COMMAND's resident half | 3,360-byte excess over retail | Produce `CODERES`/`DATARES` ranges, then move cold messages, formatting, scratch, and services into the reloadable transient |
...
The execution sequence is: finish attribution; take low-risk HIMEM and EMM386
paragraph wins; compact or relocate DOS and BIOS ranges; reduce COMMAND;
coalesce layout; then recover the EBDA paragraph. Recalculate the success
...
HIMEM occupies 2,640 bytes versus retail's 1,104. Audit its resident break at
...
| Total | 22,096 | 11,040 | 11,056 |

`MEM` reports 22,208 and 11,168 bytes after each block's arena overhead. Both
...
- investigate the remaining 1,536-byte HIMEM excess without reducing supported
  ...
  the normal HIMEM allocation by 480 bytes;
...
The current 594,592-byte largest block is 24,144 bytes (3.9%) below retail and
