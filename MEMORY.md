...
leave 605,072 bytes, a 13,664-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 2,192 |
...
| **Total** | **VC largest-block gap** | **13,664** |
...
usable UMB space. The current result is 605,072 bytes, so 13,664 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 2,192 bytes | 607,264 bytes |
...
Treat the 13,664-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 13,664 - EMM386 gain - HIMEM gain - COMMAND gain
...
| 1 | Build a small EMM386 low gateway backed by locked XMS | Local EMM386 costs 2,192 bytes more than retail; protected entry, core and handle queries, handle metadata and attributes, hardware information, mapping services, and allocation mutations are high | several kilobytes | Real/virtual transitions, inactive `AUTO`, DMA, faults, and all EMS maps |
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 11,760 bytes more than retail; after measured manager and table differences, 8,096 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `11,760 + 880 + 0 + 1,024 = 13,664`; the component census isolates 8,096 bytes for A3 |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 2,386 low bytes precede the
boundary and 15,417 protected-only bytes follow it. The report now accounts for
the complete 3,928-byte static low-image address range: a 442-byte real-mode
gateway, 272-byte GDT, 634-byte `_DATA`, 177-byte constants, 10-byte BSS,
7 bytes of alignment, and the 2,386-byte dual-mode `_TEXT` prefix. The full
...
configuration uses 8 bytes, bringing the computed installed allocation to
6,320 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 605,072 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 30,720 bytes versus retail's
18,880, an 11,840-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 11,760 bytes |
| COMMAND start through VC start | 880 bytes |
| VC start through conventional free block | 0 bytes |
| Conventional ceiling | 1,024 bytes |
| **Total** | **13,664 bytes** |

The pre-COMMAND system span contains the 2,192-byte EMM386 excess, 1,488-byte
...
| 2 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is 2,192 bytes above retail after discarding its loader stack, deriving PTE offsets, eliminating the inverse segment index, collapsing null device requests, and moving protected entry/allocation/mapping/metadata/information/query code high | Continue splitting transition, fault, DMA, and inactive-`AUTO` entries from protected/XMS state; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 is the largest component opportunity: 6,320 bytes here versus 4,128 in
...
2,192-byte low-gateway opportunity. Revisit COMMAND only as a coherent
...
| EMM386 | 6,320 | 4,128 | 2,192 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 14,704 | 11,040 | 3,664 |

`MEM` reports 14,816 and 11,168 bytes after each block's arena overhead. Both
...
The current 605,072-byte largest block is 13,664 bytes (2.2%) below retail and
