...
replacement block raises that to 3,392 bytes, still leaving **7,568 bytes**.
...
| 1 | Complete the small EMM386 low gateway backed by locked XMS | Local EMM386 matches retail after compacting its internal fast-register-set allocation flags; services, return and A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity handling, and the protected `GoVirtual` continuation execute from locked XMS; command-mode client and unused production descriptors are nonresident | create further better-than-retail headroom | Remaining transition, DMA, and fault gateways; inactive `AUTO`; all EMS maps |
...
The pre-COMMAND system span contains retail-sized EMM386, a 1,488-byte HIMEM
...
| 2 | Complete the EMM386 low gateway | The 4,128-byte allocation matches retail after fixing the inactive-mode A20 return and compacting the internal fast-register-set allocation flags; services, return and A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity, and the `GoVirtual` continuation execute from locked XMS; command-mode client code and unused production descriptors are nonresident | Redesign the remaining transition, DMA, and fault gateways for better-than-retail headroom; preserve inactive `AUTO` and every EMS map |
...
| 1 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 matches retail after preserving the inactive-mode A20 return and compacting internal fast-register-set allocation flags, while services, return/A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity handling, parity-vector state, and the `GoVirtual` continuation are high | Continue toward the smaller owner by redesigning remaining transition, fault, and DMA entries; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
5Bh save-area format and every `A=` capacity remain unchanged. EMM386 is now
4,128 bytes, the current VC largest block is 607,776 bytes, and the retail gap
is 10,960 bytes. The maximum-capacity probe now allocates all 254 optional
sets, checks exact exhaustion, and verifies reuse of low, middle, and high IDs.
OFF and `AUTO` retain their previously validated stack paragraph because their
inactive initialization path does not run VDATA compaction; this preserves all
supported modes without charging the active fixed comparison.
