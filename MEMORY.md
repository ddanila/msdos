...
595,664 bytes, a 23,072-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 9,120 |
...
| **Total** | **VC largest-block gap** | **23,072** |
...
usable UMB space. The current result is 595,664 bytes, so 23,072 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 9,120 bytes | 604,784 bytes |
...
Treat the 23,072-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 23,072 - EMM386 gain - HIMEM gain - COMMAND gain
...
3. pause incremental HIMEM work and investigate EMM386's 9,120-byte excess
...
| --- | --- | --- | ---: | --- |
| 1 | Compact EMM386 runtime-sized metadata and alignment | 9,120-byte component excess; initialization state, four tables, LOADALL scratch, and OS/E state reduced or relocated | tens to hundreds of bytes per item | Full `H=`/`A=` ranges and EMS 4.0 formats |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `18,688 + 3,360 + 0 + 1,024 = 23,072`; the component census isolates 8,096 bytes for A3 |
...
| Complete C1 | EMM386 | Produce byte-range accounting inside the current 8,094-byte low `_TEXT` prefix and 802-byte `_DATA`, including local labels, anonymous gaps, VDATA overlay, stack, and paragraph padding | Every retained byte is real-only, dual-mapped, mutable runtime, compatibility state, or unresolved |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 8,094 low bytes precede the
boundary and 9,874 protected-only bytes follow it. The report now accounts for
the complete 10,862-byte static low-image address range: a 474-byte real-mode
gateway, 272-byte GDT, 802-byte `_DATA`, 177-byte constants, 10-byte BSS,
9 bytes of alignment, the 1,024-byte linked stack template, and the 8,094-byte
...
13,248-byte installed allocation exactly matches `MEM /D`. Command-line counts
...
1. keep byte-range accounting current inside EMM386's 8,094-byte retained code
...
The validated baseline reproduces 595,664 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 37,648 bytes versus retail's
18,880, an 18,768-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 18,688 bytes |
...
The pre-COMMAND system span contains the 9,120-byte EMM386 excess, 1,488-byte
...
components occupy 21,632 of 21,744 bytes, leaving 112 bytes. Together with the
...
| ---: | --- | ---: | --- |
| 1 | Compact EMM386 low data, tables, descriptors, stack, VDATA, and alignment | 9,120-byte excess over retail | Complete byte-range ownership inside the measured low `_TEXT` and `_DATA`, then take independently testable paragraph wins |
| 2 | Relocate EMM386 protected-only code and immutable state | Included in that 9,120-byte excess | Extend the locked XMS image while leaving explicit real-mode, transition, DMA, fault, inactive-`AUTO`, and return gateways low |
...
EMM386 is the largest component opportunity: 13,248 bytes here versus 4,128 in
...
| EMM386 | 13,248 | 4,128 | 9,120 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 21,632 | 11,040 | 10,592 |

`MEM` reports 21,744 and 11,168 bytes after each block's arena overhead. Both
...
The current 595,664-byte largest block is 23,072 bytes (3.7%) below retail and
