...
the shared EMM386 dispatch table, derived physical-window PTE offsets, and the
shared bounded segment lookup leave 603,040 bytes, a 15,696-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 3,120 |
...
| **Total** | **VC largest-block gap** | **15,696** |
...
usable UMB space. The current result is 603,040 bytes, so 15,696 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 3,120 bytes | 606,160 bytes |
...
Matching only the three measured component sizes recovers at most 6,592 bytes
...
Treat the 15,696-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 15,696 - EMM386 gain - HIMEM gain - COMMAND gain
...
6,592 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 7,616 bytes, still leaving **8,080 bytes**. The final route must
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `12,688 + 1,984 + 0 + 1,024 = 15,696`; the component census isolates 8,096 bytes for A3 |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 3,276 low bytes precede the
boundary and 14,522 protected-only bytes follow it. The report now accounts for
the complete 4,850-byte static low-image address range: a 474-byte real-mode
gateway, 272-byte GDT, 634-byte `_DATA`, 177-byte constants, 10-byte BSS,
7 bytes of alignment, and the 3,276-byte dual-mode `_TEXT` prefix. The full
...
configuration uses 14 bytes, bringing the computed installed allocation to
7,248 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 603,040 versus 618,736 bytes, the component
...
EMM386 is the largest component opportunity: 7,248 bytes here versus 4,128 in
...
paired VC capture measures 603,040 bytes in the largest block and a 1,984-byte
...
| EMM386 | 7,248 | 4,128 | 3,120 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 15,632 | 11,040 | 4,592 |

`MEM` reports 15,744 and 11,168 bytes after each block's arena overhead. Both
...
The current 603,040-byte largest block is 15,696 bytes (2.5%) below retail and
