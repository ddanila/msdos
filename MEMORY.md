...
leave 606,256 bytes, a 12,480-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 1,008 |
...
| **Total** | **VC largest-block gap** | **12,480** |
...
usable UMB space. The current result is 606,256 bytes, so 12,480 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 1,008 bytes | 607,264 bytes |
...
Treat the 12,480-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 12,480 - EMM386 gain - HIMEM gain - COMMAND gain
...
3,376 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 4,400 bytes, still leaving **8,080 bytes**. The final route must
...
| 1 | Build a small EMM386 low gateway backed by locked XMS | Local EMM386 costs 1,008 bytes more than retail; services, return and A20 trap handling, and protected `RetReal` execute from locked XMS | several kilobytes | Real/virtual transitions, inactive `AUTO`, DMA, faults, and all EMS maps |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `10,576 + 880 + 0 + 1,024 = 12,480`; the component census isolates 8,096 bytes for A3 |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 1,194 low bytes precede the
boundary and 16,572 protected-only bytes follow it. The report now accounts for
the complete 2,738-byte static low-image address range: a 442-byte real-mode
gateway, 272-byte GDT, 636-byte `_DATA`, 177-byte constants, 10-byte BSS,
7 bytes of alignment, and the 1,194-byte dual-mode `_TEXT` prefix. The full
...
configuration uses 14 alignment bytes, bringing the computed installed
allocation to 5,136 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 606,256 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 29,536 bytes versus retail's
18,880, a 10,656-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 10,576 bytes |
| COMMAND start through VC start | 880 bytes |
| VC start through conventional free block | 0 bytes |
| Conventional ceiling | 1,024 bytes |
| **Total** | **12,480 bytes** |

The pre-COMMAND system span contains the 1,008-byte EMM386 excess, 1,488-byte
...
| 2 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is now 1,008 bytes above retail after moving services, return/A20 trap handling, and protected `RetReal` high | Continue toward the smaller 800-byte owner by redesigning active transition, fault, and DMA entries; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 is the largest component opportunity: 5,136 bytes here versus 4,128 in
...
1,008-byte low-gateway opportunity. Revisit COMMAND only as a coherent
...
| EMM386 | 5,136 | 4,128 | 1,008 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 13,520 | 11,040 | 2,480 |

`MEM` reports 13,632 and 11,168 bytes after each block's arena overhead. Both
...
The current 606,256-byte largest block is 12,480 bytes (2.0%) below retail and
