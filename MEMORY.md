...
configuration reports about 13.61 KiB of conventional memory for EMM386. The
...
593,968 bytes, a 24,768-byte gap.
...
| Open | Larger resident system components | 11,680 bytes |
...
usable UMB space. The current result is 593,968 bytes, so 24,768 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 9,808 bytes | 603,776 bytes |
...
| --- | --- | --- | ---: | --- |
| 1 | Compact EMM386 runtime-sized metadata and alignment | 9,808-byte component excess; initialization state and three tables reduced | tens to hundreds of bytes per item | Full `H=`/`A=` ranges and EMS 4.0 formats |
...
The validated baseline reproduces 593,968 versus 618,736 bytes, the component
...
EMM386 is the largest component opportunity: 13,936 bytes here versus 4,128 in
...
| EMM386 | 13,936 | 4,128 | 9,808 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 22,720 | 11,040 | 11,680 |

`MEM` reports 22,832 and 11,168 bytes after each block's arena overhead. Both
...
The measured system-component excess is 11,680 bytes. The current paired VC
...
The current 593,968-byte largest block is 24,768 bytes (4.0%) below retail and
