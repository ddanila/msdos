...
warm-reboot vector restoration and compact BDS flag updates leave 607,760
bytes, a 10,976-byte gap.
...
is another oversized component. The 7,600-byte DOS/BIOS row is an exact
...
usable UMB space. The current result is 607,760 bytes, so 10,976 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 0 bytes | 607,760 bytes |
| HIMEM resident allocation | 1,488 bytes | 609,248 bytes |
| COMMAND resident allocation | 880 bytes | 610,128 bytes |
| Layout and conventional ceiling | 8,608 bytes | 618,736 bytes |
...
Treat the 10,976-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 10,976 - EMM386 gain - HIMEM gain - COMMAND gain
...
raises that to 3,392 bytes, still leaving **7,584 bytes**. The final route must
therefore include at least one of these outcomes:

- recover at least 7,584 bytes from retained DOS/BIOS layout and fragmentation;
- make EMM386, HIMEM, or COMMAND collectively at least 7,584 bytes smaller than
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 9,072 bytes more than retail; after measured manager and table differences, 7,600 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `9,072 + 880 + 0 + 1,024 = 10,976`; the component census isolates 7,600 bytes for A3 |
...
1. **Finish attribution.** Extend the existing owner-level reconciliation with
   ...
   accounted; the 7,600-byte DOS/BIOS remainder still needs safe subranges.
...
3. **Reach component parity or document the irreducible difference.** Target
   ...
   block is 610,128 bytes.
4. **Recover real layout bytes.** Safely relocating the EBDA raises that
   projection to 611,152 bytes. At least another 7,584 bytes must then come from
...
the prerequisite for resolving the 8,608-byte layout row rather than moving it
between labels.
...
The current milestone reproduces 607,760 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 28,032 bytes versus retail's
18,880, a 9,152-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 9,072 bytes |
...
Subtracting those measured components leaves **7,600 bytes** of retained
...
1,700 discardable bytes. The BIOS has 15,900 linked bytes of resident-code
capacity and 20,074 discardable SYSINIT bytes; its possible hardware-selected
resident boundaries range from 8,240 bytes for floppy-only through 12,400 bytes
...
extension. `ENDONEHARD` contributes 8,332 bytes and rounds to 8,336; the
contiguous relocated 121-byte day converter and 5-byte BCD converter establish
the exact `2110h` or 8,464-byte resident BIOS boundary.
...
The fixed VC image's grouped pre-MCB payload is 15,408 bytes: the exact
6,944-byte paragraph-rounded DOS gateway plus the exact 8,464-byte selected
...
group-level MCB/gap overhead, these ranges account for the current 15,552-byte
non-component system footprint. Retail's corresponding remainder is 7,952
bytes, producing the already measured 7,600-byte excess. The likely
...
| 5 | Census and relocate eligible DOS low state | Part of the 7,600-byte DOS/BIOS remainder; 6,944 bytes allocated locally | Apply HMA, relocation-safe XMS, then bounded UMB placement according to ownership and lifetime |
| 6 in progress | Compact the selected BIOS resident image | Part of the same 7,600-byte remainder; the fixed hardware path is now exactly 8,464 bytes | Continue map-guided compaction without changing BIOS-visible services |
...
| 2 | Place mutable DOS structures high, then prefer HMA for buffers; DR-DOS 6 measures a 12,800-byte conventional move and documents `HIDOS`/`HIBUFFERS` | The local DOS/BIOS remainder is 7,600 bytes; only 1,216 bytes of UMB advantage may be spent before falling below retail | Classify each owner, then use HMA, relocation-safe XMS, and finally deterministic UMB placement; filesystem, device, redirector, EXEC, A20-off, and rollback paths must pass |
...
inferring layout from component totals. The current 8,608-byte row may contain
...
The current 607,760-byte largest block is 10,976 bytes (1.8%) below retail and
