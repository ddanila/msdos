...
594,688 bytes, a 24,048-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 9,488 |
...
| **Total** | **VC largest-block gap** | **24,048** |
...
usable UMB space. The current result is 594,688 bytes, so 24,048 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 9,488 bytes | 604,176 bytes |
...
Matching only the three measured component sizes recovers at most 14,336 bytes
...
Treat the 24,048-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 24,048 - EMM386 gain - HIMEM gain - COMMAND gain
...
14,336 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 15,360 bytes, still leaving **8,688 bytes**. The final route must
...
3. pause incremental HIMEM work and investigate EMM386's 9,488-byte excess
...
| --- | --- | --- | ---: | --- |
| 1 | Compact EMM386 runtime-sized metadata and alignment | 9,488-byte component excess; initialization state, four tables, LOADALL scratch, and OS/E state reduced or relocated | tens to hundreds of bytes per item | Full `H=`/`A=` ranges and EMS 4.0 formats |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `19,664 + 3,360 + 0 + 1,024 = 24,048`; the component census isolates 8,704 bytes for A3 |
...
the complete 11,156-byte static low-image address range: a 474-byte real-mode
...
The validated baseline reproduces 594,688 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 38,624 bytes versus retail's
18,880, a 19,744-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 19,664 bytes |
...
The pre-COMMAND system span contains the 9,488-byte EMM386 excess, 1,488-byte
...
components occupy 22,000 of 22,112 bytes, leaving 112 bytes. Together with the
...
| ---: | --- | ---: | --- |
| 1 | Compact EMM386 low data, tables, descriptors, stack, VDATA, and alignment | 9,488-byte excess over retail | Complete byte-range ownership inside the measured low `_TEXT` and `_DATA`, then take independently testable paragraph wins |
| 2 | Relocate EMM386 protected-only code and immutable state | Included in that 9,488-byte excess | Extend the locked XMS image while leaving explicit real-mode, transition, DMA, fault, inactive-`AUTO`, and return gateways low |
...
EMM386 is the largest component opportunity: 13,616 bytes here versus 4,128 in
...
| EMM386 | 13,616 | 4,128 | 9,488 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 22,000 | 11,040 | 10,960 |

`MEM` reports 22,112 and 11,168 bytes after each block's arena overhead. Both
...
The current 594,688-byte largest block is 24,048 bytes (3.9%) below retail and
