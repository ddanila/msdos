...
production GDT, and selected-BIOS compaction leave 607,328 bytes, an
11,408-byte gap.
...
is another oversized component. The 8,032-byte DOS/BIOS row is an exact
...
usable UMB space. The current result is 607,328 bytes, so 11,408 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 0 bytes | 607,328 bytes |
| HIMEM resident allocation | 1,488 bytes | 608,816 bytes |
| COMMAND resident allocation | 880 bytes | 609,696 bytes |
| Layout and conventional ceiling | 9,040 bytes | 618,736 bytes |
...
Treat the 11,408-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 11,408 - EMM386 gain - HIMEM gain - COMMAND gain
...
raises that to 3,392 bytes, still leaving **8,016 bytes**. The final route must
therefore include at least one of these outcomes:

- recover at least 8,016 bytes from retained DOS/BIOS layout and fragmentation;
- make EMM386, HIMEM, or COMMAND collectively at least 8,016 bytes smaller than
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 9,504 bytes more than retail; after measured manager and table differences, 8,032 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `9,504 + 880 + 0 + 1,024 = 11,408`; the component census isolates 8,032 bytes for A3 |
...
1. **Finish attribution.** Extend the existing owner-level reconciliation with
   ...
   accounted; the 8,032-byte DOS/BIOS remainder still needs safe subranges.
...
3. **Reach component parity or document the irreducible difference.** Target
   ...
   block is 609,696 bytes.
4. **Recover real layout bytes.** Safely relocating the EBDA raises that
   projection to 610,720 bytes. At least another 8,016 bytes must then come from
...
the prerequisite for resolving the 9,040-byte layout row rather than moving it
between labels.
...
The current milestone reproduces 607,328 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 28,464 bytes versus retail's
18,880, a 9,584-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 9,504 bytes |
...
Subtracting those measured components leaves **8,032 bytes** of retained
...
1,700 discardable bytes. The BIOS has 16,284 linked bytes of resident-code
...
extension. `ENDONEHARD` contributes 8,672 bytes; the contiguous relocated
165-byte day converter and 5-byte BCD converter establish the exact `2290h` or
8,848-byte resident BIOS boundary.
...
The fixed VC image's grouped pre-MCB payload is 15,840 bytes: the exact
6,992-byte paragraph-rounded DOS gateway plus the exact 8,848-byte selected
...
group-level MCB/gap overhead, these ranges account for the current 15,984-byte
non-component system footprint. Retail's corresponding remainder is 7,952
bytes, producing the already measured 8,032-byte excess. The likely
...
| 5 | Census and relocate eligible DOS low state | Part of the 8,032-byte DOS/BIOS remainder; 6,992 bytes allocated locally | Apply HMA, relocation-safe XMS, then bounded UMB placement according to ownership and lifetime |
| 6 in progress | Compact the selected BIOS resident image | Part of the same 8,032-byte remainder; the fixed hardware path is now exactly 8,848 bytes | Continue map-guided compaction without changing BIOS-visible services |
...
| 2 | Place mutable DOS structures high, then prefer HMA for buffers; DR-DOS 6 measures a 12,800-byte conventional move and documents `HIDOS`/`HIBUFFERS` | The local DOS/BIOS remainder is 8,032 bytes; only 1,216 bytes of UMB advantage may be spent before falling below retail | Classify each owner, then use HMA, relocation-safe XMS, and finally deterministic UMB placement; filesystem, device, redirector, EXEC, A20-off, and rollback paths must pass |
...
stores adjacent century/year bytes together, makes the division remainder
one-based, and relies on the normalized one-year count to bound the month loop.
...
separate entry pointers. The helpers shrink from 210 and 15 bytes to 165 and 5
bytes; the fixed BIOS image falls from 8,912 to 8,848 bytes. All four released
paragraphs join VC's largest block, raising it to 607,328 bytes and reducing the
retail gap to 11,408 bytes. DATE handling, SYS, interrupt, HMA, and 286/386/486
local gates pass; the residency check enforces the new 8,848-byte ceiling.
...
inferring layout from component totals. The current 9,040-byte row may contain
...
The current 607,328-byte largest block is 11,408 bytes (1.8%) below retail and
