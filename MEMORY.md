...
is another oversized component. The 5,776-byte DOS/BIOS row is an exact
owner-level remainder. Phase A3 now accounts for every byte of both the
5,392-byte DOS low prefix and the selected 8,192-byte BIOS image. E1 must turn
...
usable UMB space. The current result is 609,776 bytes, so 8,960 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | -192 bytes | 609,776 bytes |
| HIMEM resident allocation | 1,488 bytes | 611,248 bytes |
| COMMAND resident allocation | 880 bytes | 612,128 bytes |
| Layout and conventional ceiling | 6,592 bytes | 618,736 bytes |
...
Treat the 8,960-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 8,960 - EMM386 gain - HIMEM gain - COMMAND gain
...
replacement block raises that to 3,392 bytes, still leaving **5,568 bytes**.
The final route must
therefore include at least one of these outcomes:

- recover at least 5,584 bytes from retained DOS/BIOS layout and fragmentation;
- make EMM386, HIMEM, or COMMAND collectively at least 5,632 bytes smaller than
...
5. **In progress:** EMM386 now retains a 404-byte dual-mode `_TEXT` gateway;
...
6. **In progress:** the complete 783-byte DOS system-call dispatcher and the
...
| 3 | Classify and relocate eligible DOS low state | The dispatcher and 652-byte absolute/system/INT 2Fh tranche are high; the local system-to-COMMAND span now costs 7,056 bytes more than retail and leaves 5,776 bytes of measured DOS/BIOS remainder | low kilobytes | Classify the remaining tables and mutable state; some low addresses are ABI/BIOS fixed |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `7,056 + 880 + 0 + 1,024 = 8,960`; the component census isolates 5,776 bytes for E1 |
| Complete A3 | Measurement | Add reproducible HIMEM, EMM386, COMMAND, DOS, and BIOS resident-range reports from their linker maps | HIMEM, EMM386, COMMAND top-level lifetimes, fixed HMA layout, and every byte of the 5,392-byte DOS low prefix and selected 8,192-byte BIOS image are accounted |
...
| Complete C1 | EMM386 | Maintain byte-range accounting for the current 404-byte low `_TEXT` prefix and 424-byte `_DATA`, including local labels, anonymous gaps, VDATA overlay, discarded loader stack, and paragraph padding | Every retained byte is real-only, dual-mapped, mutable runtime, compatibility state, discarded initialization state, or unresolved |
...
the complete 1,528-byte static low-image address range: a 336-byte real-mode
...
computed retained-layout end to 3,952 bytes. The paired live image reports a
3,936-byte EMM386 payload, 192 bytes below retail; keep the map coordinate and
live allocation distinct.
Command-line counts
...
The current milestone reproduces 609,776 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 26,016 bytes versus retail's
18,880, a 7,136-byte difference that includes the memory managers and retained
...
For the current build, `DOS_LOW_GATE_END` is 5,381 linked and 5,392
paragraph-rounded bytes below the HMA. The HMA copy contains 39,440 bytes
after its fixed `0010h` entry offset, and DOS's `LAST` initialization segment is
1,795 discardable bytes. The BIOS has 15,628 linked bytes of resident-code
capacity and 20,161 discardable SYSINIT bytes; its possible hardware-selected
resident boundaries range from 7,968 bytes for floppy-only through 12,128 bytes
...
extension. `ENDONEHARD` contributes and rounds to 8,064 bytes; the
contiguous relocated 121-byte day converter and 5-byte BCD converter establish
the exact `2000h` or 8,192-byte resident BIOS boundary.

The DOS low prefix consists of 3 bytes of loader entry, 1,072 bytes of
constants, 1,783 bytes of mutable data, 2,365 bytes of tables, 157 bytes of
...
The selected BIOS census likewise covers all 8,192 bytes. Its largest ranges
...
2,024 bytes cover console, auxiliary, printer, clock, model/vector, disk-init,
...
DOS=LOW. The complete 783-byte system-call dispatcher contribution, including
its 12-byte EMS map, remains linked above that tranche and copied to HMA.

The fixed VC image's grouped pre-MCB payload is 13,584 bytes: the exact
5,392-byte paragraph-rounded DOS gateway plus the exact 8,192-byte selected
BIOS image. The following system MCB contains 12,432 bytes; enumerated
components occupy 12,320 bytes, leaving 112 bytes. Together with 32 bytes of
group-level MCB/gap overhead, these ranges account for the current 13,728-byte
non-component system footprint. Retail's corresponding remainder is 7,952
bytes, producing the already measured 5,776-byte excess. The likely
...
| 2 | Compact EMM386 metadata while changing that boundary | The loader stack is discarded, duplicate PTE offsets and inverse segment index are gone, physical-window segments, public `Pn=` identifiers, DMA pages, mappable-window indexes, bounded counters, and parity-vector state are runtime-sized, narrowed, derived, shared, or protected-high; protected entry/services/helpers are high, the two-function control gateway is table-free, the production GDT omits six debugger and six unused legacy descriptors, and `NOHIMEM` hooks are absent. A tested bundle removed ten cached low-data bytes (`_regp` segment, free-page count, saved-map and sparse-page pointers, context size, and frame-page count), but enlarged protected code and left both the 3,936-byte live owner and VC unchanged | The baseline selected tail ends at raw `0D68h`; require a coherent reduction to `0D50h` or below—24 raw bytes—before retaining another metadata tranche. Prefer gateway work unless a bundle meets that measured boundary without a material speed penalty |
| 5 in progress | Census and relocate eligible DOS low state | The dispatcher and contiguous 652-byte absolute/system/INT 2Fh tranche are HMA-resident under DOS=HIGH, reducing the DOS gateway to 5,392 bytes and the DOS/BIOS remainder to 5,776 bytes | Classify the remaining tables and mutable state, then apply HMA, relocation-safe XMS, and bounded UMB placement |
| 6 in progress | Compact the selected BIOS resident image | Part of the same 5,776-byte remainder; the fixed hardware path is now 8,192 bytes, its warm-boot vector table uses its existing segment sentinels, its PS/2 result-preservation gate passes, and its six-DPB store reuses the existing two-record DOS low reserve | Continue map-guided compaction without changing BIOS-visible services |
...
| 1 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | Active EMM386 is 192 bytes below retail after the real IDTR move and VM-frame scratch compaction around a packed atomic `IRET`; services, return/A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity handling, parity-vector state, and the `GoVirtual` continuation are high | Continue toward the smaller owner by redesigning remaining transition, fault, and DMA entries; all EMS maps, modes, shifted loads, and warm reboot must pass |
| 2 | Place mutable DOS structures high, then prefer HMA for buffers; DR-DOS 6 measures a 12,800-byte conventional move and documents `HIDOS`/`HIBUFFERS` | The dispatcher and absolute/system/INT 2Fh tranche are high and the local DOS/BIOS remainder is 5,776 bytes; only 1,216 bytes of UMB advantage may be spent before falling below retail | Classify each remaining owner, then use HMA, relocation-safe XMS, and finally deterministic UMB placement; filesystem, device, redirector, EXEC, A20-off, and rollback paths must pass |
...
Active EMM386 now occupies 3,936 bytes, 192 below retail. Potential further
...
relocatable code byte, while the complete 783-byte dispatcher contribution is
...
| EMM386 | 3,936 | 4,128 | -192 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 12,320 | 11,040 | 1,280 |

`MEM` reports 12,432 and 11,168 bytes after each block's arena overhead. Both
...
The current 609,776-byte largest block is 8,960 bytes (1.4%) below retail and
