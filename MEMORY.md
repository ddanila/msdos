...
configuration reports about 13.625 KiB of conventional memory for EMM386. The
...
593,952 bytes, a 24,784-byte gap.
...
| Open | Larger resident system components | 11,696 bytes |
...
usable UMB space. The current result is 593,952 bytes, so 24,784 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 9,824 bytes | 603,776 bytes |
...
| --- | --- | --- | ---: | --- |
| 1 | Compact EMM386 runtime-sized metadata and alignment | 9,824-byte component excess; three table reductions already measured | tens to hundreds of bytes per item | Full `H=`/`A=` ranges and EMS 4.0 formats |
...
The validated baseline reproduces 593,952 versus 618,736 bytes, the component
...
EMM386 is the largest component opportunity: 13,952 bytes here versus 4,128 in
...
| EMM386 | 13,952 | 4,128 | 9,824 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 22,736 | 11,040 | 11,696 |

`MEM` reports 22,848 and 11,168 bytes after each block's arena overhead. Both
...
The measured system-component excess is 11,696 bytes. The current paired VC
...
The current 593,952-byte largest block is 24,784 bytes (4.0%) below retail and
