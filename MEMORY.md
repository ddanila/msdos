...
dispatcher flag path leave 607,408 bytes, an 11,328-byte gap.
...
is another oversized component. The 7,952-byte DOS/BIOS row is an exact
...
usable UMB space. The current result is 607,408 bytes, so 11,328 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 0 bytes | 607,408 bytes |
| HIMEM resident allocation | 1,488 bytes | 608,896 bytes |
| COMMAND resident allocation | 880 bytes | 609,776 bytes |
| Layout and conventional ceiling | 8,960 bytes | 618,736 bytes |
...
Treat the 11,328-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 11,328 - EMM386 gain - HIMEM gain - COMMAND gain
...
raises that to 3,392 bytes, still leaving **7,936 bytes**. The final route must
therefore include at least one of these outcomes:

- recover at least 7,936 bytes from retained DOS/BIOS layout and fragmentation;
- make EMM386, HIMEM, or COMMAND collectively at least 7,936 bytes smaller than
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 9,424 bytes more than retail; after measured manager and table differences, 7,952 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `9,424 + 880 + 0 + 1,024 = 11,328`; the component census isolates 7,952 bytes for A3 |
| A3 in progress | Measurement | Add reproducible HIMEM, EMM386, COMMAND, DOS, and BIOS resident-range reports from their linker maps | HIMEM, EMM386, COMMAND top-level lifetimes, fixed HMA layout, selected BIOS, and all 1,601 bytes of retained DOS CODE are accounted; DOS constants and tables remain coarse |
...
1. **Finish attribution.** Extend the existing owner-level reconciliation with
   ...
   accounted; the 7,952-byte DOS/BIOS remainder still needs safe subranges.
...
3. **Reach component parity or document the irreducible difference.** Target
   ...
   block is 609,776 bytes.
4. **Recover real layout bytes.** Safely relocating the EBDA raises that
   projection to 610,800 bytes. At least another 7,936 bytes must then come from
...
the prerequisite for resolving the 8,960-byte layout row rather than moving it
between labels.
...
The current milestone reproduces 607,408 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 28,384 bytes versus retail's
18,880, a 9,504-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 9,424 bytes |
...
Subtracting those measured components leaves **7,952 bytes** of retained
...
For the current build, `DOS_LOW_GATE_END` is 6,951 linked bytes and consumes
6,960 paragraph-rounded bytes below the HMA. The HMA copy contains 39,584 bytes
after its fixed `0010h` entry offset, and DOS's `LAST` initialization segment is
1,700 discardable bytes. The BIOS has 16,238 linked bytes of resident-code
...
extension. `ENDONEHARD` contributes 8,670 bytes and is paragraph-aligned to
8,672; the contiguous relocated 121-byte day converter and 5-byte BCD converter
establish the exact `2260h` or 8,800-byte resident BIOS boundary.

The DOS low prefix consists of 3 bytes of loader entry, 1,072 bytes of
constants, 1,783 bytes of mutable data, 2,491 bytes of tables, 1,601 bytes of
...
bytes of EMS user-map storage, 778 bytes of core system-call dispatch, 70 bytes of HMA
...
largest next classification targets are therefore the 778-byte dispatcher and
...
The fixed VC image's grouped pre-MCB payload is 15,760 bytes: the exact
6,960-byte paragraph-rounded DOS gateway plus the exact 8,800-byte selected
...
group-level MCB/gap overhead, these ranges account for the current 15,904-byte
non-component system footprint. Retail's corresponding remainder is 7,952
bytes, producing the already measured 7,952-byte excess. The likely
...
| 5 | Census and relocate eligible DOS low state | Part of the 7,952-byte DOS/BIOS remainder; 6,960 bytes allocated locally | Apply HMA, relocation-safe XMS, then bounded UMB placement according to ownership and lifetime |
| 6 in progress | Compact the selected BIOS resident image | Part of the same 7,952-byte remainder; the fixed hardware path is now exactly 8,800 bytes | Continue map-guided compaction without changing BIOS-visible services |
...
| 2 | Place mutable DOS structures high, then prefer HMA for buffers; DR-DOS 6 measures a 12,800-byte conventional move and documents `HIDOS`/`HIBUFFERS` | The local DOS/BIOS remainder is 7,952 bytes; only 1,216 bytes of UMB advantage may be spent before falling below retail | Classify each owner, then use HMA, relocation-safe XMS, and finally deterministic UMB placement; filesystem, device, redirector, EXEC, A20-off, and rollback paths must pass |
...
rejected. The code adds 112 bytes to the HMA image but leaves the current 6,960-byte low
...
inferring layout from component totals. The current 8,960-byte row may contain
...
The current 607,408-byte largest block is 11,328 bytes (1.8%) below retail and
