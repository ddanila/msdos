...
the protected transition then releases two reported paragraphs: 608,032 bytes
and a 10,704-byte gap.
...
| --- | --- | ---: |
| Advantage | EMM386 resident allocation | -128 |
...
| **Total** | **VC largest-block gap** | **10,704** |
...
usable UMB space. The current result is 608,032 bytes, so 10,704 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | -128 bytes | 608,032 bytes |
| HIMEM resident allocation | 1,488 bytes | 609,520 bytes |
| COMMAND resident allocation | 880 bytes | 610,400 bytes |
| Layout and conventional ceiling | 8,336 bytes | 618,736 bytes |
...
Treat the 10,704-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 10,704 - EMM386 gain - HIMEM gain - COMMAND gain
...
replacement block raises that to 3,392 bytes, still leaving **7,312 bytes**.
The final route must
therefore include at least one of these outcomes:

- recover at least 7,312 bytes from retained DOS/BIOS layout and fragmentation;
- make EMM386, HIMEM, or COMMAND collectively at least 7,360 bytes smaller than
...
| 1 | Complete the small EMM386 low gateway backed by locked XMS | Local active EMM386 is 128 bytes below retail after the real IDTR load moved immediately before the protected-mode exit; the required FS/GS and interrupt-state contracts remain intact | create further better-than-retail headroom | Remaining transition, DMA, and fault gateways; inactive `AUTO`; all EMS maps |
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 8,800 bytes more than retail; after measured manager and table differences, 7,456 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `8,800 + 880 + 0 + 1,024 = 10,704`; the component census isolates 7,456 bytes for E1 |
...
the complete 1,592-byte static low-image address range: a 393-byte real-mode
gateway, 176-byte GDT, 424-byte `_DATA`, 177-byte constants, 10-byte BSS,
8 bytes of alignment, and the 404-byte dual-mode `_TEXT` prefix. The full
...
computed retained-layout end to 4,016 bytes. The paired live image reports a
4,000-byte EMM386 payload, 128 bytes below retail; keep the map coordinate and
...
The current milestone reproduces 608,032 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 27,760 bytes versus retail's
18,880, an 8,880-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 8,800 bytes |
| COMMAND start through VC start | 880 bytes |
| VC start through conventional free block | 0 bytes |
| Conventional ceiling | 1,024 bytes |
| **Total** | **10,704 bytes** |

The pre-COMMAND system span contains EMM386's 128-byte advantage, a 1,488-byte
...
BIOS image. The following system MCB contains 12,496 bytes; enumerated
components occupy 12,384 bytes, leaving 112 bytes. Together with 32 bytes of
...
| 2 | Complete the EMM386 low gateway | The active 4,000-byte allocation is 128 bytes below retail after loading the real IDTR immediately before clearing PE; the required FS/GS and interrupt-state contracts remain intact | Redesign the remaining transition, DMA, and fault gateways for further headroom; preserve inactive `AUTO` and every EMS map |
...
| 1 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | Active EMM386 is 128 bytes below retail after loading the real IDTR immediately before clearing PE; services, return/A20/NMI trap handling, protected `RetReal`, stack-selector conversion, OEM mapping/parity handling, parity-vector state, and the `GoVirtual` continuation are high | Continue toward the smaller owner by redesigning remaining transition, fault, and DMA entries; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
Active EMM386 now occupies 4,000 bytes, 128 below retail. Potential further
...
remain intact. `_TEXT` falls from 428 to 404 bytes and the static image to 1,592
bytes. The map's retained-layout end is 4,016 bytes, while the paired live image
reports a 4,000-byte EMM386 payload and a 608,032-byte VC block: two reported
paragraphs gained and a 10,704-byte retail gap. A shared far-return convention
for `GoVirtual` hung the full function 56h probe and was rejected; do not retry
it without a mapping-aware return design. The complete EMS, option, address,
frame, UMB, reboot, and 286/386/486 hardware gates pass locally.
...
| EMM386 | 4,000 | 4,128 | -128 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 12,384 | 11,040 | 1,344 |

`MEM` reports 12,496 and 11,168 bytes after each block's arena overhead. Both
...
The current 608,032-byte largest block is 10,704 bytes (1.7%) below retail and
