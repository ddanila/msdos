...
the shared EMM386 dispatch table, and derived physical-window PTE offsets leave
603,024 bytes, a 15,712-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 3,136 |
...
| Open | COMMAND owner-to-owner span | 1,984 |
| Equal | VC owner-to-free span | 0 |
| Open | Conventional ceiling/EBDA | 1,024 |
| **Total** | **VC largest-block gap** | **15,712** |
...
usable UMB space. The current result is 603,024 bytes, so 15,712 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 3,136 bytes | 606,160 bytes |
| HIMEM resident allocation | 1,488 bytes | 607,648 bytes |
| COMMAND resident allocation | 1,984 bytes | 609,632 bytes |
| Layout and conventional ceiling | 9,104 bytes | 618,736 bytes |

Matching only the three measured component sizes recovers at most 6,608 bytes
...
Treat the 15,712-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 15,712 - EMM386 gain - HIMEM gain - COMMAND gain
...
6,608 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 7,632 bytes, still leaving **8,080 bytes**. The final route must
...
4. **In progress:** COMMAND's complete 1,281-byte normal resident utility and
   ...
   compiled-catalog capacity bound. These changes recover 1,376
   paragraph-rounded bytes while leaving 16,603 bytes for later HMA payloads.
...
| 1 | Move COMMAND cold state high or transient | All 1,281 normal resident catalog bytes are high; COMMAND still costs 1,984 bytes more than retail and 3,824 more than DR-DOS | low kilobytes | Reload, critical error, `INT 2Eh`, batch and pipe survival |
| 2 | Build a small EMM386 low gateway backed by locked XMS | Local EMM386 costs 3,136 bytes more than retail; protected mapping services and allocation mutations are high | several kilobytes | Real/virtual transitions, inactive `AUTO`, DMA, faults, and all EMS maps |
...
| 3 | Classify and relocate eligible DOS low state | The local system span costs 12,704 bytes more than retail and 20,976 more than DR-DOS | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `12,704 + 1,984 + 0 + 1,024 = 15,712`; the component census isolates 8,096 bytes for A3 |
...
| Complete C1 | EMM386 | Produce byte-range accounting inside the current 3,246-byte low `_TEXT` prefix and 694-byte `_DATA`, including local labels, anonymous gaps, VDATA overlay, discarded loader stack, and paragraph padding | Every retained byte is real-only, dual-mapped, mutable runtime, compatibility state, discarded initialization state, or unresolved |
...
| D1 in progress | COMMAND | Refine the generated `CODERES`/`DATARES` byte-range census from lifetime classes to symbol/module ownership | The checked report now anchors eight permanent-low ranges totaling the exact 4,478 bytes after the PSP; the first caller audit removes eight dead save/restore bytes from the 2,065-byte error/message-service range |
...
COMMAND census to continue splitting the 2,065-byte resident error/message-service range,
...
The current map divides `_TEXT` at `IOTrap_Tab`: 3,246 low bytes precede the
boundary and 14,528 protected-only bytes follow it. The report now accounts for
the complete 4,880-byte static low-image address range: a 474-byte real-mode
gateway, 272-byte GDT, 694-byte `_DATA`, 177-byte constants, 10-byte BSS,
7 bytes of alignment, and the 3,246-byte dual-mode `_TEXT` prefix. The full
...
configuration needs no gap, bringing the computed installed allocation to
7,264 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 603,024 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 31,664 bytes versus retail's
18,880, a 12,784-byte difference that includes the memory managers and retained
DOS layout. COMMAND contributes another 1,984 bytes, while the one-kilobyte
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 12,704 bytes |
| COMMAND start through VC start | 1,984 bytes |
| VC start through conventional free block | 0 bytes |
| Conventional ceiling | 1,024 bytes |
| **Total** | **15,712 bytes** |

The pre-COMMAND system span contains the 3,136-byte EMM386 excess, 1,488-byte
...
components occupy 15,648 of 15,760 bytes, leaving 112 bytes. Together with the
...
| ---: | --- | ---: | --- |
| 1 | Continue the COMMAND/HMA census | The normal resident catalog range is high; COMMAND remains 1,984 bytes above retail and 3,824 above DR-DOS | Attribute and move the next coherent cold range without weakening fallback paths |
...
| 3 | Extend the EMM386 low gateway | 3,136-byte excess over retail; protected mapping and allocation services execute from locked XMS | Move the next coherent service family while leaving save-map/query, transition, DMA, fault, inactive-`AUTO`, and return gateways low; preserve every EMS map |
| 4 | Compact EMM386 metadata while changing that boundary | The loader stack is discarded, duplicate PTE offsets are gone, and mapping/allocation code is high; remaining work is included in the 3,136-byte excess | Take independently testable descriptor, VDATA, table, and alignment wins without delaying the gateway design |
...
| 2 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is 3,136 bytes above retail after discarding its loader stack, deriving PTE offsets, and moving allocation and mapping services high | Continue splitting query, transition, fault, DMA, and inactive-`AUTO` entries from protected/XMS state; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 is the largest component opportunity: 7,264 bytes here versus 4,128 in
...
COMMAND now occupies 4,944 bytes versus retail's 2,960. Its retained allocation
...
local `make test`. The permanent DOS-high break is 4,734 bytes, rounded to
4,736: a 256-byte PSP, 3,561 bytes of resident code, a 125-byte resident stack,
562 bytes of mutable shell state, and 230 bytes of mutable message-runtime data.
...
low; that fallback break is
6,015 bytes, rounded to 6,016. The 1,000-byte parse/extended catalog tail is
...
entry/EXEC/reload paths, 2,065 bytes of error and message services, a 125-byte
...
the 2,065-byte service range by callers and asynchronous-entry requirements;
...
The permanent root shell now reserves 1,281 bytes from that tail and copies the
normal resident utility and critical-error catalog range to `FFFF:BA14h`. The
...
free tail address is `BF15h`, leaving 16,603 bytes. Local QEMU startup, child
batch/pipe, HMA, 286 86Box, and 286/386/486 hardware-matrix tests pass. The latest
paired VC capture measures 603,024 bytes in the largest block and a 1,984-byte
...
| EMM386 | 7,264 | 4,128 | 3,136 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 15,648 | 11,040 | 4,608 |

`MEM` reports 15,760 and 11,168 bytes after each block's arena overhead. Both
...
capture reports COMMAND at 4,944 bytes versus retail's 2,960, a separate
1,984-byte excess. The retained boundaries are:
...
- COMMAND's 4,944-byte footprint remains above retail's 2,960 bytes. Its
  DOS-high root break is 4,736 paragraph-rounded program bytes after moving the
  normal resident catalog range to HMA, bounding the class table, moving
  redirection state into initialized transient data, and bounding the message
  workspace; low/failure fallback remains 6,016 bytes. Closing the remaining 1,984 bytes
  requires further
...
The current 603,024-byte largest block is 15,712 bytes (2.5%) below retail and
