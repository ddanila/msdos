...
594,752 bytes, a 23,984-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 9,424 |
...
| **Total** | **VC largest-block gap** | **23,984** |
...
usable UMB space. The current result is 594,752 bytes, so 23,984 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 9,424 bytes | 604,176 bytes |
...
Matching only the three measured component sizes recovers at most 14,272 bytes
...
Treat the 23,984-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 23,984 - EMM386 gain - HIMEM gain - COMMAND gain
...
14,272 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 15,296 bytes, still leaving **8,688 bytes**. The final route must
...
3. pause incremental HIMEM work and investigate EMM386's 9,424-byte excess
...
| --- | --- | --- | ---: | --- |
| 1 | Compact EMM386 runtime-sized metadata and alignment | 9,424-byte component excess; initialization state, four tables, LOADALL scratch, and OS/E state reduced or relocated | tens to hundreds of bytes per item | Full `H=`/`A=` ranges and EMS 4.0 formats |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `19,600 + 3,360 + 0 + 1,024 = 23,984`; the component census isolates 8,704 bytes for A3 |
| A3 in progress | Measurement | Add reproducible HIMEM, EMM386, COMMAND, DOS, and BIOS resident-range reports from their linker maps | HIMEM, EMM386, and DOS/BIOS boundaries are reported; COMMAND and byte-range ownership remain |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 8,186 low bytes precede the
boundary and 9,889 protected-only bytes follow it. The report now accounts for
the complete 11,162-byte static low-image address range: a 474-byte real-mode
gateway, 272-byte GDT, 1,010-byte `_DATA`, 177-byte constants, 10-byte BSS,
9 bytes of alignment, the 1,024-byte linked stack template, and the 8,186-byte
...
6 bytes of final alignment and the 512-byte protected stack, the computed
13,552-byte installed allocation exactly matches `MEM /D`. Command-line counts
...
The validated baseline reproduces 594,752 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 38,560 bytes versus retail's
18,880, a 19,680-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 19,600 bytes |
...
The pre-COMMAND system span contains the 9,424-byte EMM386 excess, 1,488-byte
...
components occupy 21,936 of 22,048 bytes, leaving 112 bytes. Together with the
...
| ---: | --- | ---: | --- |
| 1 | Compact EMM386 low data, tables, descriptors, stack, VDATA, and alignment | 9,424-byte excess over retail | Complete byte-range ownership inside the measured low `_TEXT` and `_DATA`, then take independently testable paragraph wins |
| 2 | Relocate EMM386 protected-only code and immutable state | Included in that 9,424-byte excess | Extend the locked XMS image while leaving explicit real-mode, transition, DMA, fault, inactive-`AUTO`, and return gateways low |
...
| 4 | Select and compact the BIOS resident boundary | Part of the same 8,704-byte remainder; 8,912 bytes including inferred boundary/alignment | Identify the exact hardware-selected end, remove unused legacy blocks where configuration permits, and compact without changing BIOS-visible services |
...
EMM386 is the largest component opportunity: 13,552 bytes here versus 4,128 in
...
| EMM386 | 13,552 | 4,128 | 9,424 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 21,936 | 11,040 | 10,896 |

`MEM` reports 22,048 and 11,168 bytes after each block's arena overhead. Both
...
The current 594,752-byte largest block is 23,984 bytes (3.9%) below retail and
