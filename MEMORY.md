...
leave 606,064 bytes, a 12,672-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 1,200 |
...
| **Total** | **VC largest-block gap** | **12,672** |
...
usable UMB space. The current result is 606,064 bytes, so 12,672 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 1,200 bytes | 607,264 bytes |
...
Matching only the three measured component sizes recovers at most 3,568 bytes
...
Treat the 12,672-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 12,672 - EMM386 gain - HIMEM gain - COMMAND gain
...
3,568 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 4,592 bytes, still leaving **8,080 bytes**. The final route must
...
| 1 | Build a small EMM386 low gateway backed by locked XMS | Local EMM386 costs 1,200 bytes more than retail; the service dispatcher, EMS services, OS/E control, shared helpers, return-trap initializer/dispatcher, and protected half of `RetReal` are high | several kilobytes | Real/virtual transitions, inactive `AUTO`, DMA, faults, and all EMS maps |
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 10,768 bytes more than retail; after measured manager and table differences, 8,096 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `10,768 + 880 + 0 + 1,024 = 12,672`; the component census isolates 8,096 bytes for A3 |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 1,397 low bytes precede the
boundary and 16,373 protected-only bytes follow it. The report now accounts for
the complete 2,941-byte static low-image address range: a 442-byte real-mode
gateway, 272-byte GDT, 636-byte `_DATA`, 177-byte constants, 10-byte BSS,
7 bytes of alignment, and the 1,397-byte dual-mode `_TEXT` prefix. The full
...
configuration uses three alignment bytes, bringing the computed installed
allocation to 5,328 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 606,064 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 29,728 bytes versus retail's
18,880, a 10,848-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 10,768 bytes |
| COMMAND start through VC start | 880 bytes |
| VC start through conventional free block | 0 bytes |
| Conventional ceiling | 1,024 bytes |
| **Total** | **12,672 bytes** |

The pre-COMMAND system span contains the 1,200-byte EMM386 excess, 1,488-byte
...
| 1 | Extend the EMM386 low gateway | 1,200-byte excess over retail; the dispatcher, protected EMS services, OS/E control, shared helpers, return-trap initializer/dispatcher, and protected `RetReal` body execute from locked XMS | Redesign the remaining active transition, DMA, fault, and return gateways; preserve every EMS map |
| 1 | Compact EMM386 metadata while changing that boundary | The loader stack is discarded, duplicate PTE offsets and inverse segment index are gone, protected entry/services/helpers are high, and duplicate null requests are gone; remaining work is included in the 1,200-byte excess | Take independently testable descriptor, VDATA, table, and alignment wins without delaying the gateway design |
...
| 2 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is now exactly 1,200 bytes above retail after discarding its loader stack, deriving PTE offsets, eliminating the inverse segment index, collapsing null requests, and moving the dispatcher, services, OS/E control, helpers, return-trap initializer/dispatcher, and protected `RetReal` body high | Continue toward the smaller 800-byte owner by redesigning active transition, fault, and DMA entries; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 is the largest component opportunity: 5,328 bytes here versus 4,128 in
...
1,200-byte low-gateway opportunity. Revisit COMMAND only as a coherent
...
| EMM386 | 5,328 | 4,128 | 1,200 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 13,712 | 11,040 | 2,672 |

`MEM` reports 13,824 and 11,168 bytes after each block's arena overhead. Both
...
The current 606,064-byte largest block is 12,672 bytes (2.1%) below retail and
