...
production GDT, and selected-BIOS compaction leave 607,376 bytes, an
11,360-byte gap.
...
is another oversized component. The 7,984-byte DOS/BIOS row is an exact
...
usable UMB space. The current result is 607,376 bytes, so 11,360 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 0 bytes | 607,376 bytes |
| HIMEM resident allocation | 1,488 bytes | 608,864 bytes |
| COMMAND resident allocation | 880 bytes | 609,744 bytes |
| Layout and conventional ceiling | 8,992 bytes | 618,736 bytes |
...
Treat the 11,360-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 11,360 - EMM386 gain - HIMEM gain - COMMAND gain
...
raises that to 3,392 bytes, still leaving **7,968 bytes**. The final route must
therefore include at least one of these outcomes:

- recover at least 7,968 bytes from retained DOS/BIOS layout and fragmentation;
- make EMM386, HIMEM, or COMMAND collectively at least 7,968 bytes smaller than
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 9,456 bytes more than retail; after measured manager and table differences, 7,984 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
1. **Finish attribution.** Extend the existing owner-level reconciliation with
   ...
   accounted; the 7,984-byte DOS/BIOS remainder still needs safe subranges.
...
3. **Reach component parity or document the irreducible difference.** Target
   ...
   block is 609,744 bytes.
4. **Recover real layout bytes.** Safely relocating the EBDA raises that
   projection to 610,768 bytes. At least another 7,968 bytes must then come from
...
the prerequisite for resolving the 8,992-byte layout row rather than moving it
between labels.
...
The current milestone reproduces 607,376 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 28,416 bytes versus retail's
18,880, a 9,536-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 9,456 bytes |
...
Subtracting those measured components leaves **7,984 bytes** of retained
...
group-level MCB/gap overhead, these ranges account for the current 15,936-byte
non-component system footprint. Retail's corresponding remainder is 7,952
bytes, producing the already measured 7,984-byte excess. The likely
...
| 5 | Census and relocate eligible DOS low state | Part of the 7,984-byte DOS/BIOS remainder; 6,992 bytes allocated locally | Apply HMA, relocation-safe XMS, then bounded UMB placement according to ownership and lifetime |
| 6 in progress | Compact the selected BIOS resident image | Part of the same 7,984-byte remainder; the fixed hardware path is now exactly 8,800 bytes | Continue map-guided compaction without changing BIOS-visible services |
...
| 2 | Place mutable DOS structures high, then prefer HMA for buffers; DR-DOS 6 measures a 12,800-byte conventional move and documents `HIDOS`/`HIBUFFERS` | The local DOS/BIOS remainder is 7,984 bytes; only 1,216 bytes of UMB advantage may be spent before falling below retail | Classify each owner, then use HMA, relocation-safe XMS, and finally deterministic UMB placement; filesystem, device, redirector, EXEC, A20-off, and rollback paths must pass |
...
inferring layout from component totals. The current 8,992-byte row may contain
...
The current 607,376-byte largest block is 11,360 bytes (1.8%) below retail and
