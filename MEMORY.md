...
COMMAND HMA relocation, and the discarded EMM386 initialization stack leaves
597,872 bytes, a 20,864-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 7,376 |
...
| Open | COMMAND owner-to-owner span | 2,896 |
| Equal | VC owner-to-free span | 0 |
| Open | Conventional ceiling/EBDA | 1,024 |
| **Total** | **VC largest-block gap** | **20,864** |
...
usable UMB space. The current result is 597,872 bytes, so 20,864 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 7,376 bytes | 605,248 bytes |
| HIMEM resident allocation | 1,488 bytes | 606,736 bytes |
| COMMAND resident allocation | 2,896 bytes | 609,632 bytes |
| Layout and conventional ceiling | 9,104 bytes | 618,736 bytes |

Matching only the three measured component sizes recovers at most 11,760 bytes
...
Treat the 20,864-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 20,864 - EMM386 gain - HIMEM gain - COMMAND gain
...
11,760 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 12,784 bytes, still leaving **8,080 bytes**. The final route must
...
| 1 | Move COMMAND cold state high or transient | The first 475-byte catalog is high; COMMAND still costs 2,896 bytes more than retail and 4,736 more than DR-DOS | low kilobytes | Reload, critical error, `INT 2Eh`, batch and pipe survival |
| 2 | Build a small EMM386 low gateway backed by locked XMS | Local EMM386 costs 7,376 bytes more than retail; OpenDOS demonstrates a much smaller reported low resident | several kilobytes | Real/virtual transitions, inactive `AUTO`, DMA, faults, and all EMS maps |
...
| 3 | Classify and relocate eligible DOS low state | The local system span costs 16,944 bytes more than retail and 25,216 more than DR-DOS | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `16,944 + 2,896 + 0 + 1,024 = 20,864`; the component census isolates 8,096 bytes for A3 |
...
| Complete C1 | EMM386 | Produce byte-range accounting inside the current 7,383-byte low `_TEXT` prefix and 798-byte `_DATA`, including local labels, anonymous gaps, VDATA overlay, discarded loader stack, and paragraph padding | Every retained byte is real-only, dual-mapped, mutable runtime, compatibility state, discarded initialization state, or unresolved |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 7,383 low bytes precede the
boundary and 10,446 protected-only bytes follow it. The report now accounts for
the complete 9,111-byte static low-image address range: a 474-byte real-mode
gateway, 272-byte GDT, 798-byte `_DATA`, 177-byte constants, 10-byte BSS,
7 bytes of alignment, and the 7,383-byte dual-mode `_TEXT` prefix. The full
...
allocation to 11,504 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 597,872 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 35,904 bytes versus retail's
18,880, a 17,024-byte difference that includes the memory managers and retained
DOS layout. COMMAND contributes another 2,896 bytes, while the one-kilobyte
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 16,944 bytes |
| COMMAND start through VC start | 2,896 bytes |
| VC start through conventional free block | 0 bytes |
| Conventional ceiling | 1,024 bytes |
| **Total** | **20,864 bytes** |

The pre-COMMAND system span contains the 7,376-byte EMM386 excess, 1,488-byte
...
components occupy 19,888 of 20,000 bytes, leaving 112 bytes. Together with the
...
| ---: | --- | ---: | --- |
| 1 | Continue the COMMAND/HMA census | The first catalog is high; COMMAND remains 2,896 bytes above retail and 4,736 above DR-DOS | Attribute and move the next coherent cold range without weakening fallback paths |
...
| 3 | Extend the EMM386 low gateway | 7,376-byte excess over retail; allocation mutations now execute from locked XMS, and OpenDOS reports only an 800-byte UMB owner | Move the next coherent service family while leaving query, transition, DMA, fault, inactive-`AUTO`, and return gateways low; preserve every EMS map |
| 4 | Compact EMM386 metadata while changing that boundary | The full-depth initialization stack is discarded and allocation code is high; remaining work is included in the 7,376-byte excess | Take independently testable descriptor, VDATA, table, and alignment wins without delaying the gateway design |
...
| 2 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is 7,376 bytes above retail after discarding its loader stack and moving allocation mutations high | Continue splitting query, transition, fault, DMA, inactive-`AUTO`, and mapping-sensitive entries from protected/XMS state; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 is the largest component opportunity: 11,504 bytes here versus 4,128 in
...
COMMAND now occupies 5,856 bytes versus retail's 2,960. Its retained allocation
...
local `make test`. The permanent DOS-high break is 5,641 bytes, rounded to
5,648: a 256-byte PSP, 3,553 bytes of resident code, a 125-byte resident stack,
643 bytes of mutable shell state, and 1,064 bytes of resident message-service
...
| EMM386 | 11,504 | 4,128 | 7,376 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 19,888 | 11,040 | 8,848 |

`MEM` reports 20,000 and 11,168 bytes after each block's arena overhead. Both
...
capture reports COMMAND at 5,856 bytes versus retail's 2,960, a separate
2,896-byte excess. The retained boundaries are:
...
- COMMAND's 5,856-byte footprint remains above retail's 2,960 bytes. Its
  DOS-high root break is 5,648 paragraph-rounded program bytes after moving the
  critical catalog to HMA; low/failure fallback remains 6,128 bytes. Closing
  the remaining 2,896 bytes requires further targeted resident-shell work, not
  correction of an accidentally retained transient tail.
...
The current 597,872-byte largest block is 20,864 bytes (3.4%) below retail and
