...
| **Total** | **VC largest-block gap** | **8,928** |
...
is another oversized component. The 5,744-byte DOS/BIOS row is an exact
owner-level remainder. Phase A3 now accounts for every byte of both the
5,392-byte DOS low prefix and the selected 8,160-byte BIOS image. E1 must turn
...
usable UMB space. The current result is 609,808 bytes, so 8,928 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | -192 bytes | 609,808 bytes |
| HIMEM resident allocation | 1,488 bytes | 611,296 bytes |
| COMMAND resident allocation | 880 bytes | 612,176 bytes |
| Layout and conventional ceiling | 6,560 bytes | 618,736 bytes |
...
Treat the 8,928-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 8,928 - EMM386 gain - HIMEM gain - COMMAND gain
...
replacement block raises that to 3,392 bytes, still leaving **5,536 bytes**.
The final route must
therefore include at least one of these outcomes:

- recover at least 5,552 bytes from retained DOS/BIOS layout and fragmentation;
- make EMM386, HIMEM, or COMMAND collectively at least 5,600 bytes smaller than
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `7,024 + 880 + 0 + 1,024 = 8,928`; the component census isolates 5,744 bytes for E1 |
...
The current milestone reproduces 609,808 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 25,984 bytes versus retail's
18,880, a 7,104-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 7,024 bytes |
...
The current 609,808-byte largest block is 8,928 bytes (1.4%) below retail and
