...
598,144 bytes, a 20,592-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 7,104 |
...
| **Total** | **VC largest-block gap** | **20,592** |
...
usable UMB space. The current result is 598,144 bytes, so 20,592 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 7,104 bytes | 605,248 bytes |
...
Matching only the three measured component sizes recovers at most 11,488 bytes
...
Treat the 20,592-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 20,592 - EMM386 gain - HIMEM gain - COMMAND gain
...
11,488 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 12,512 bytes, still leaving **8,080 bytes**. The final route must
...
| 2 | Build a small EMM386 low gateway backed by locked XMS | Local EMM386 costs 7,104 bytes more than retail; OpenDOS demonstrates a much smaller reported low resident | several kilobytes | Real/virtual transitions, inactive `AUTO`, DMA, faults, and all EMS maps |
...
| 3 | Classify and relocate eligible DOS low state | The local system span costs 16,672 bytes more than retail and 24,944 more than DR-DOS | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `16,672 + 2,896 + 0 + 1,024 = 20,592`; the component census isolates 8,096 bytes for A3 |
...
| Complete C1 | EMM386 | Produce byte-range accounting inside the current 7,111-byte low `_TEXT` prefix and 798-byte `_DATA`, including local labels, anonymous gaps, VDATA overlay, discarded loader stack, and paragraph padding | Every retained byte is real-only, dual-mapped, mutable runtime, compatibility state, discarded initialization state, or unresolved |
...
The current map divides `_TEXT` at `IOTrap_Tab`: 7,111 low bytes precede the
boundary and 10,716 protected-only bytes follow it. The report now accounts for
the complete 8,839-byte static low-image address range: a 474-byte real-mode
gateway, 272-byte GDT, 798-byte `_DATA`, 177-byte constants, 10-byte BSS,
7 bytes of alignment, and the 7,111-byte dual-mode `_TEXT` prefix. The full
...
allocation to 11,232 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 598,144 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 35,632 bytes versus retail's
18,880, a 16,752-byte difference that includes the memory managers and retained
...
| Span | Difference |
| --- | ---: |
| System start through COMMAND start | 16,672 bytes |
...
The pre-COMMAND system span contains the 7,104-byte EMM386 excess, 1,488-byte
...
components occupy 19,616 of 19,728 bytes, leaving 112 bytes. Together with the
...
| 3 | Extend the EMM386 low gateway | 7,104-byte excess over retail; all allocation mutations now execute from locked XMS, and OpenDOS reports only an 800-byte UMB owner | Move the next coherent service family while leaving query, transition, DMA, fault, inactive-`AUTO`, and return gateways low; preserve every EMS map |
| 4 | Compact EMM386 metadata while changing that boundary | The full-depth initialization stack is discarded and allocation code is high; remaining work is included in the 7,104-byte excess | Take independently testable descriptor, VDATA, table, and alignment wins without delaying the gateway design |
...
| 2 | Retain only a small conventional/UMB gateway for the 386 memory manager; OpenDOS reports a 1,200-byte conventional device range and an 800-byte UMB owner | EMM386 is 7,104 bytes above retail after discarding its loader stack and moving allocation mutations high | Continue splitting query, transition, fault, DMA, inactive-`AUTO`, and mapping-sensitive entries from protected/XMS state; all EMS maps, modes, shifted loads, and warm reboot must pass |
...
EMM386 is the largest component opportunity: 11,232 bytes here versus 4,128 in
...
| EMM386 | 11,232 | 4,128 | 7,104 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 19,616 | 11,040 | 8,576 |

`MEM` reports 19,728 and 11,168 bytes after each block's arena overhead. Both
...
The current 598,144-byte largest block is 20,592 bytes (3.3%) below retail and
