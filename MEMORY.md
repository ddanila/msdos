...
lists and protected parity-vector state leave 607,200 bytes, an 11,536-byte gap.
...
| --- | --- | ---: |
| Open | EMM386 resident excess | 64 |
...
| **Total** | **VC largest-block gap** | **11,536** |
...
usable UMB space. The current result is 607,200 bytes, so 11,536 more bytes must
...
| --- | ---: | ---: |
| EMM386 resident allocation | 64 bytes | 607,264 bytes |
...
Matching only the three measured component sizes recovers at most 2,432 bytes
...
Treat the 11,536-byte gap as a portfolio, not as four independent size targets.
For every retained change record:

```text
remaining gap = 11,536 - EMM386 gain - HIMEM gain - COMMAND gain
...
2,432 bytes. Moving the 1 KiB EBDA without allocating a replacement block
raises that to 3,456 bytes, still leaving **8,080 bytes**. The final route must
...
| 3 | Classify and relocate eligible DOS low state | The local system-to-COMMAND span costs 9,632 bytes more than retail; after measured manager and table differences, 8,096 bytes remain DOS/BIOS-owned | low kilobytes | Some low addresses are ABI/BIOS fixed; HMA/XMS/UMB lifetimes differ |
...
| Complete A2 | Measurement | Reconcile the full VC gap through system, COMMAND, VC, free-block, and ceiling spans, isolating the retained DOS/BIOS remainder | The current report proves `9,632 + 880 + 0 + 1,024 = 11,536`; the component census isolates 8,096 bytes for A3 |
...
| Complete C1 | EMM386 | Maintain byte-range accounting for the current 483-byte low `_TEXT` prefix and 438-byte `_DATA`, including local labels, anonymous gaps, VDATA overlay, discarded loader stack, and paragraph padding | Every retained byte is real-only, dual-mapped, mutable runtime, compatibility state, discarded initialization state, or unresolved |
...
boundary and 17,430 non-low bytes follow it. The report now accounts for
the complete 1,765-byte static low-image address range: a 426-byte real-mode
gateway, 224-byte GDT, 438-byte `_DATA`, 177-byte constants, 10-byte BSS,
...
`_DATA` into driver/messages, EMS tables, OEM runtime, DMA, and move-state ranges.
...
allocation to 4,192 bytes, exactly matching `MEM /D`.
Command-line counts
...
The current milestone reproduces 607,200 versus 618,736 bytes, the component
...
VC snapshot, the grouped `DOS 6.22` payload is 28,592 bytes versus retail's
18,880, a 9,712-byte difference that includes the memory managers and retained
...
EMM386 now occupies 4,192 bytes here versus 4,128 in retail. Potential further
...
| EMM386 | 4,192 | 4,128 | 64 |
| FILES | 896 | 896 | 0 |
| FCBS | 256 | 256 | 0 |
| BUFFERS | 512 | 512 | 0 |
| LASTDRIVE | 2,288 | 2,288 | 0 |
| STACKS | 1,840 | 1,856 | -16 |
| Total | 12,576 | 11,040 | 1,536 |

`MEM` reports 12,688 and 11,168 bytes after each block's arena overhead. Both
...
The current 607,200-byte largest block is 11,536 bytes (1.9%) below retail and
